...
last_updated: "2026-09-23T10:25:56.971Z"
...
state_head: 7cff0244f78d2ef2244bfc82b5960d5f074d3d40
progress:
  total_phases: 27
  completed_phases: 4
  total_plans: 233
...
Status: 라운드 4 실행 완료 · 재검증(-R4) 대기 · relay 미배포 — 18-36 전량 게이트 green(build_command error TS 0 · relay 534 · webapp 1472/1 skip · Playwright 143/0 fail/9 skip · GC1~GC6 · R4 DB 변경 0 → pgTAP 미실행) · 18-VALIDATION §Gap Closure R4 7행 전부 닫힘 · escalation 2건 응답 · TRADE-06~09 Pending 유지 · 배포 순서 DB(완료 · R4 변경 0) → relay(R2 18-14·17·19 + R3 18-25·18-26 + R4 18-33) → 검증 → webapp push (18-26 webapp 은 relay 18-26 뒤에만 · R4 webapp 새 결합 없음)
Production URL: https://gh-radar-webapp.vercel.app
Last activity: 2026-09-23 — Completed quick task 260923-que: ✕ 결과 모름 경고에서 다른 카드가 보여주는 키 제외 / 이전: Completed quick task 260923-pq2: NXT 미거래 종목 세그먼트 KRX 하나(relay nxt.snap, relay 미배포) / 이전: Completed quick task 260923-pgv: 카드 거래소 토글 잠금 해제(키 전환 · 더티 확인 · ✕ 두 키 판정) / 이전: Completed quick task 260923-pgu: 작업대 이벤트 알림 토스트(③A · D-27 갱신) / 이전: Completed quick task 260923-p3k: 작업대 카드 순서(접힘/펼침 무리의 끝으로 · withCardOpen) / 이전: Completed quick task 260923-onn: 작업대 카드 안 탭(정보|미체결|잔고|로그) + 접힌 카드 요약 칩(LED 점 3개 · 미체결 N · 잔고 N주) — 목업 ①A/②A / 이전: Completed quick task 260923-m23: 정정확인 수량 캡 대응 — dma_orders 정정 모델링(qty 캡 · modified_qty · modified 상태, 마이그레이션 원격 미적용) / 이전: Completed quick task 260923-ge2: 가벼운 시세 구독(SubscribeQuoteReq.level PRICE) 적용 — relay hub full/price 참조계수 · 브라우저 sub lv · 돌파 칩 price 구독(CPU 2b) / 이전: Completed quick task 260923-elb: /trading CPU 1단계(q/tape 100ms 배치 · 테이프 1벌 · 돌파 칩 2Hz — S3 99→14%) / 이전: Completed quick task 260923-dmb: 사이드바 VI 한 줄 + VI 발동 목록 최신순 / 이전: Completed quick task 260923-cqj: relay 게이트웨이 종목마스터 27/57 보조 원천(신규상장 종목명, relay 미배포) / 이전: Completed quick task 260923-e1m: relay WS-order 통보 기록 경로 수정(공유세션 수동통보 info · 자동취소 원주문 cancelled · 체결 조각 누적) / 이전: Completed quick task 260923-cre: 오늘의 급등테마 개별 급등 급등이유 복사(카드별 아이콘 · 섹션 전체 복사) / 이전: 래치 LED 보조 문구 「(매도잔량 기준)」·「(발주됨)」 제거(fast) / 이전: Completed quick task 260923-bjb: 작업대 상단 정리(VI 설정 펼침 안 한 줄 · 상태줄 핵심만 · 펼침 머리줄/개수 제거) / 이전: Completed quick task 260922-uhw: R4-WR-01 결과 모름 잠금 교차 거래소 틈 수정 / 이전: Completed quick task 260922-tqr: 작업대/호가 UI 정리 3건(마감알림 제거 · 검색 입력 확대 방지 · 체결 테이프 설명 제거) / 이전: 18-36 R4 최종 게이트(relay 534 · webapp 1472/1 skip · Playwright 143/0 fail/9 skip · GC1~GC6 · R4 DB 변경 0) · 18-VALIDATION §Gap Closure R4(7행 · 대응표 · escalation 응답 · Manual-Only 10 · 배포 순서 R4) · REQUIREMENTS footer R4(Pending 유지) · deferred handshake(R3-IN-05) / 이전: 18-35 잠금 단일 원천(18-30 배선 제거) · 카드 정리 cards 파생 · isOffhoursOrder 주석 / 18-34 결과 모름 잠금 RelayProvider 앱 수명 · 신규·정정만 · 진행 중 포함 · e2e GC6 / 18-33 relay 상태 단조성 조건부 UPDATE(ws ㊹ ㊺ RED→GREEN) / 18-32 R3 최종 게이트(relay 523 · webapp 1454/1 skip · Playwright 142/0 fail/9 skip · GC1~GC5 · 로컬 DB 회귀 12/12 · 20260922180000 Local=Remote) · 18-VALIDATION §Gap Closure R3 · REQUIREMENTS footer R3(Pending 유지) · deferred 해소 / 이전: 18-31 GC-IN-05 fillAccountCards 펼침 승계 · forgetCardState 단일 정리 경로 / GC-IN-01 dirtyBarCount 재측정 · strategy-card 주석 분리 / GC-IN-04 holdingQuotePrice(KRX 우선·NXT 폴백) · exchangeLabeledName(로그 who · 사이드바)(RED→GREEN, webapp 1454 · e2e 40) / 이전: 18-29 GC-CR-01 원격 dma_orders_price_check null-safe 적용(위반 행 0 사전 확인 · 사용자 apply · 전후 덤프 diff CHECK 1줄 · 정책 0/0 · RLS·GRANT 불변) / 이전: 18-30 GC-WR-03 결과 모름 잠금을 TradingWorkbench 계좌|ISIN|거래소 키 집합으로(페이지 언마운트만 해제) · 잠긴 카드 ✕ 결과 모름 다이얼로그 · e2e GC5(webapp 1435 · e2e 45) / 이전: 18-27 GC-WR-02·GC-IN-06·GC-IN-03 취소 확정 현재 잔량 내림 · 재대조 문구 분리 · isOffhoursOrder(RED→GREEN, webapp 1399) / 이전: 18-26 GC-IN-02 relay 는 64 를 받았을 때만 인증 경로 lc.snap(⑭(B)·⑭-4·⑯ RED→GREEN) · 작업대 포커스 보류 = 이번 연결 확정 스냅샷(①②③ RED→GREEN)
...
| 260923-iwz | **상따 폼 디자인 정리 (목업 승인 후)** — 3284bea 가 목업 없이 나가 망친 디자인 수정. 게이트 체크박스를 그룹 제목 **왼쪽**(CheckRow 와 같은 x)으로, 상태 점(LED) 제거 · 매수/매도 방향색 배경: 폰 탭 화면은 옵션 pane 가장자리까지(같은 색 `box-shadow` spread 8px — 음수 마진은 e2e 잘림 검사에 걸림), 2열은 둥근 모서리 + 8px 여백, 데스크톱 카드 테두리 · 수동주문 2×2 유지. 목업 `260923-iwz-mockup.html`(라이트/다크). tsc 0 · vitest trading 652 · Playwright 작업대+a11y+호가 59/0 | 2026-09-23 | 9fb39f3 | [260923-iwz-led-pane-box-shadow-spread-2-2-2](./quick/260923-iwz-led-pane-box-shadow-spread-2-2-2/) |
| 260923-hfk | **갤럭시 폴드(wb ≈ 691)에서 단 수 세그먼트 사라짐 (사용자 스크린샷)** — 격자 열 수 경계를 카드 밴드 700 에서 분리해 wb 680 으로(`COLS_CLASS`·상태줄 CSS 폴백·`WB_SINGLE_COLUMN_BELOW` 세 곳 동일, §2.2b 「격자 열 수 경계」 문단). 공용 패널·표 열 접기의 wb 700 은 무변경. tsc 0 · 유닛 통과. 카드 폭 사다리 실측은 생략(680 → 카드 334px 계산값), 폴드 실기기 확인 대기 | 2026-09-23 | 2087f18 | [260923-hfk-691px-trading-1-2-3-wb-700-2-2-gap](./quick/260923-hfk-691px-trading-1-2-3-wb-700-2-2-gap/) |
| 260923-ge2 | **가벼운 시세 구독(SubscribeQuoteReq.level PRICE) 적용 — CPU 2b** — gh-trade 228ea1f5(quick-260923-exo) 생성 코드 2파일 커밋 · `buildSubscribeQuoteReq` level 인자(`QUOTE_LEVEL` FULL 0/PRICE 1) · hub 참조계수를 키당 {full, price} 로 나눠 실효 level(full≥1 ? FULL : PRICE) 로 29 송신 — 0→1 PRICE 는 28→29(1), FULL 은 28→29(0)→32, PRICE→FULL 승격은 3프레임 재송신, FULL→PRICE 강등은 29(1) 1건, 마지막 이탈 29(false), ready 재구독은 실효 level · 브라우저 `sub` 에 `lv?: full\|price`(열거 밖 4400), 같은 소켓 재구독은 level 갱신(새 level 먼저 올리고 옛 level 내림), price 소켓에는 tape 미전송(relay 자체 보장) · webapp `subscribe/unsubscribe(isin, ex, level="full")` 합성·실효 level 바뀌면 sub 재송신(full 프레임 모양 불변), `useBreakoutQuotes` 만 price. shared `RelaySubLevel`. relay 604 · webapp 1516/1 skip · typecheck 0 · Playwright trading-workbench+orderbook 49/0. 게이트웨이 가동본은 level 모르는 구버전(재기동 전) → PRICE 효과는 재기동 뒤. 배포 순서 relay → webapp push. 관찰: 69/71 중복 제거 없음(후속) · 76/78 exchange 가 NXT 일 수 있음(돌파 훅 KRX 고정 점검 후속) | 2026-09-23 | 3718b36 | [260923-ge2-subscribequotereq-level-price-gh-trade-2](./quick/260923-ge2-subscribequotereq-level-price-gh-trade-2/) |
| 260923-elb | **/trading 브라우저 CPU 1단계 (debug trading-cpu-260923)** — ① 시세·체결(q/tape) 프레임을 `use-relay-socket` 에서 100ms 배치로 모아 한 커밋에 적용(그 밖 프레임은 배치 먼저 flush, 순서 보존) ② 상따 사다리의 체결 테이프를 3 트리 안 사본 대신 트리 밖 1벌로, 행 키 안정화(`tapeRowKeys`)·행 memo·고정 시 50행 렌더 창 ③ 돌파 칩 가격 ≤2Hz(`BREAKOUT_PRICE_THROTTLE_MS` 500) · 렌더 중 setState 제거(`advanceTracked`). 메인스레드 S1 33.7→14% · S2 40~46→13~15% · S3(분산 도착) 99→14%(렌더러 121→24%) · S4 45~49→20~22% · DOM 17.1k→6.7k. webapp 1510/1 skip(+23 RED→GREEN) · Playwright 64/0. relay/shared 무변경. 남은 과제 1b 컨텍스트 분리 · 2b PRICE 구독(→ 260923-ge2) · 3b 활성 트리만 렌더 | 2026-09-23 | 62d8a77 | [260923-elb-cpu-1](./quick/260923-elb-cpu-1/) |
| 260923-dmb | **사이드바 VI 한 줄 + VI 발동 목록 최신순 (사용자 요청)** — ① 사이드바 트레이딩 아래 KRX VI·NXT VI 두 줄을 「VI」 한 줄(`data-sidebar-item="vi"`)로 합치고 가동 중인 거래소 태그(`ExchangeTag`, `vi-order-list` 재export)만 오른쪽에, 둘 다 꺼져 있으면 줄 자체 숨김 ② 작업대 VI 발동 목록 `sortViOrdersNewestFirst`(`deadline110Ms` 내림차순 · 동률은 orderNo 유무 → orderNo 내림차순 → `viOrderKey`)를 리듀서 `vi.list` 에 적용 — 접힘은 최신이 맨 왼쪽 칩, 펼침은 맨 위 행. webapp 1487 · Playwright 53/0 | 2026-09-23 | a46131f | [260923-dmb-vi-vi](./quick/260923-dmb-vi-vi/) |
| 260923-cqj | **relay 게이트웨이 종목마스터(27/57) 를 Supabase stocks 미스 보조 원천으로 (신규상장 ISIN 표시 분석 후속)** — 스크린샷의 ISIN 표시 원인: `SymbolMap` 이 Supabase `stocks` 를 ISIN 으로만 적재하고 KRX OpenAPI 는 전영업일 자료를 다음 영업일 08:00 에 내놓아 당일 신규상장은 하루 늦음(intraday bootstrap 행은 `isin: null`). 새 `store/gateway-symbols.ts` 가 57 분할 프레임(500/프레임 · seq 0 부터 연속 · `total_items` 일치 · 상한 20k/1k/100 seq)을 조립해 원자 교체, `SymbolMap` 은 Supabase 행 우선·미색인 ISIN 만 게이트웨이 항목(시장 "0"→K/"1"→Q/그 밖 null, `source:"gateway"`), 주문 감사 행 `stock_code` 는 게이트웨이 출처면 null(FK `stocks(code)`). 57 은 INBOUND 로. 타이밍: 07:30 KST 경계 타이머 · 30s 타임아웃 · 5분 재시도 · 하루 5회 상한 · `SessionManager.firstReady`. `hub.refreshNames()` 가 이름 없이 나간 캐시 행 재방송. relay 564. **relay 미배포** — 20:00 이후 `scripts/deploy-relay.sh`(DMA_HOST 없이) → `[SYM-GW]` 로그 · `/healthz` 확인 | 2026-09-23 | 51be413 | [260923-cqj-relay-27-57-supabase-stocks](./quick/260923-cqj-relay-27-57-supabase-stocks/) |
| 260923-e1m | **relay WS-order 통보 기록 경로 수정 (debug relay-ws-order-unmatched-notice)** — A1 공유 게이트웨이 세션(D-17 철회)으로 온 WinForms 수동 통보는 relay 흔적(대기 후보·timeout/연결종료 종목·정산 주문번호)과 무관하면 info 강등, relay 관련만 ERROR 유지 · B1 자동주문 취소확인 C 는 orgOrderNo(없으면 orderNo) 로 원주문 행을 찾아 cancelled, 거부 R 은 원주문 유지·info · B3 취소성 통보는 insert 금지(autoInsertRow 가드·낡은 price CHECK 주석 정정) · S1 대기 밖 E 는 filledQtyDelta 로 fill sink(CAS 조건부 UPDATE, 3회 상한)가 누적·filled/partially_filled 파생. 마이그레이션·RPC 없음. relay 586/586 · typecheck 0. 배포 20:00 이후 relay 단독 → push | 2026-09-23 | de80122 | [260923-e1m-relay-ws-order-info](./quick/260923-e1m-relay-ws-order-info/) |
| 260923-cre | **오늘의 급등테마 「개별 급등」 급등이유 복사 (사용자 요청)** — SoloCard 등락% 오른쪽 32px 복사 아이콘(`{종목명} 급등이유 복사`, `{종목명} {+x.x%}` + 이유 있을 때만 한 줄 · 이유 없어도 버튼 렌더 · 오버레이 Link 형제+z-20 이라 상세 이동 없음) · 「개별 급등」 제목 행 「전체 복사」(`[개별 급등] {tradeDate} {KST HH:MM}` + 화면 순서 번호 블록). `formatSingleBlock`·`formatSinglesSummary` 추가, 주도 테마 헤더와 `snapshotHeader` 공유(주도 테마 출력 불변) · CopyTextButton 무변경. webapp 1479/1 skip · typecheck · lint 0 error · Playwright home 11/11(실 Chromium 클립보드·force 없는 클릭·390 한 줄) | 2026-09-23 | cf3a665 | [260923-cre-copy-solo-surge-reasons](./quick/260923-cre-copy-solo-surge-reasons/) |
| 260923-fast | 상따 카드 헤더 래치 LED 보조 문구 「(매도잔량 기준)」·「(발주됨)」 제거 — 칩이 길어 ⓘ·✕ 가 다음 줄로 내려가던 문제(사용자 스크린샷). 칩은 매수/매도/취소 + OFF·대기·감시만, 클릭 불가 사유는 툴팁 원문 유지. `LatchLedState.note`·`LatchLedServer.hadOrder` 제거 · 유닛 772 · Playwright trading-workbench+sidebar-tree 43/0. 함께: 체결 테이프 최신 행 플래시 배경 `--fg` 14% → 5%(아주 옅은 회색, 사용자 요청) · orderbook/stock 유닛 263 | 2026-09-23 | (이 커밋) | — |
| 260923-bjb | **작업대 상단 정리 (사용자 요청 3건 · 목업 승인)** — ① KRX/NXT VI 설정을 VI 패널 「더보기」 안으로(접힘은 `hidden` 유지라 더티 보존) · 거래소당 한 줄, 본문 ≥830 은 KRX \| NXT 한 줄 · 「가동중/중지/서버 반영」 문구 제거 ② 상태줄 = DMA · 77 구간 배지 · 알림음 아이콘 · 반영 시각 · 단 수만(돌파/VI발동/거래 종목 개수 · 임계/재무장 문구 제거) ③ 펼친 VI·돌파 머리줄 · VI 표 설명문 · 접힌 줄 개수 제거, 빈 목록은 표 없음. VI 서버 거절 alert 는 접혀도 보이게 패널 안 상단. webapp 1467/1 skip · Playwright trading-workbench+a11y 45/0 · 390 한 줄(28px)·1000 나란히 실측 | 2026-09-23 | 0b06230 | [260923-bjb-workbench-top-cleanup](./quick/260923-bjb-workbench-top-cleanup/) |
| 260922-uhw | **R4-WR-01 수정 — 결과 모름 잠금 교차 거래소 틈(18-34 회귀)** — 호가 탭 KRX 폼에서 NXT 미체결 정정 timeout 뒤 4버튼이 다시 열리던 문제. `formOrderLockOf` 로 폼 키 ∪ 선택 행 키 ∪ 마지막 전송 키(같은 종목·계좌 한정, 취소 비기억) 중 하나라도 잠기면 잠금, 결과 모름 > 진행 중. 원천은 `RelayProvider.orderLocks` 유지. RED(2 failed) → GREEN, 변이 5종 확인, webapp 1460/1 skip · Playwright 47/0 · typecheck 0. 18-VALIDATION §Gap Closure R4 종결 기록 | 2026-09-22 | 1a38078 | [260922-uhw-r4-wr-01-cross-exchange-result-unknown-l](./quick/260922-uhw-r4-wr-01-cross-exchange-result-unknown-l/) |
| 260922-tqr | **작업대/호가 UI 정리 3건** — ① 작업대 상태줄 VI 「마감알림」 토글 기능째 제거(`ViAlertToggle`·`useViEndAlerts`·`use-vi-end-alerts.ts` 삭제, `vi-alert.ts` 알림 전용 export 정리 — 금액 변환·`isViServerMessage` 유지) ② iPhone 종목검색 입력 포커스 확대 방지 — `command.tsx`·`stock-add-bar.tsx` 입력을 `text-base pointer-fine:text-sm`(터치 16px·마우스 14px, 가로모드 재발 방지로 `sm:` 대신 `pointer-fine:`), viewport 확대 금지 미사용 ③ 체결 테이프 하단 「수량 색…」 설명과 상따 카드 중복 캡션 제거(매수/매도 판정 로직 불변). webapp 1456/1 skip · Playwright 54/0 · typecheck 0. 돌파감지 ISIN 표시는 코드 결함 아님 — 프로덕션 relay `ef1499a` 가 18-01 종목명 보강 이전이라 relay 배포로 해소 | 2026-09-22 | 00b7e4c | [260922-tqr-workbench-ui-cleanup](./quick/260922-tqr-workbench-ui-cleanup/) |
...
| 260915-h3p | **news-sync 수집 주기 재설계 — 장시간 단일 3분 잡·장외 스케줄 분리·일일 한도 소진 중단·CLAUDE.md 공식 API 기준** — 실측: Naver 검색 API 소비는 사실상 news-sync 단독(평일 약 4.8K·주말 1.25K), 1회 ≈ 대상 104 + 추가 페이지, 429 12건 전부 짝수 정각 offhours 동시 발화. (1) `cadence.ts`: 평일 KST 08:00~20:02 tiered(급등 rank≤30 ∪ 관심종목 매회 + 나머지 top_movers 코드 해시 3조 중 `floor(분/3)%3` 조 → 9분 주기, stateless), 그 외 full, `NEWS_SYNC_MODE` override, 예산 24,500→18,750(75%). (2) 429 본문으로 일일 한도(010 가정)·초당(012 가정) 구분 + 5종목 연속 429 휴리스틱 → run 즉시 중단, 같은 KST 날짜 두 번째 판정부터 자정까지 skip(`naver_search_news_quota_strike`). (3) 스케줄러 market `*/3 8-19`·market-close `0 20`·offhours `0 0-6/2,22 * * 1-5`·weekend `0 */2 * * 0,6`, morning·morning-10h·intraday 삭제, 1주 분 단위 전개 테스트로 동분 중복 0. (4) CLAUDE.md·STACK.md: 스크래핑 5원칙과 공식 API 운영 기준(75% 예산·주기 폴링 허용·사용자 수 독립·소진 시 당일 중단·출처) 분리. 게이트: news-sync vitest 128 통과·typecheck·build, 배포 smoke 11/11, 라이브 12:51/12:54/12:57 버킷 2→0→1·호출 54/65/57. 미관측: 20:00·22:00·주말 실행, 실제 429 본문 코드. 범위 밖: KRX 평일 휴장일은 평일 취급, server `NAVER_DAILY_BUDGET` 불변 | 2026-09-15 | 993c6a7 | [260915-h3p-news-sync-tiered-cadence](./quick/260915-h3p-news-sync-tiered-cadence/) |
| 260915-il4 | **news-sync 한도 소진 당일 중단 제거 — 판정 run 만 중단 + 다음 run 단독 탐침 호출로 자동 복구** — h3p 의 "같은 KST 날짜 strike 2회 → 자정까지 skip" 은 판정 두 경로(429 본문 010/'query limit exceeded' 가정 · 5종목 연속 429 휴리스틱)가 초당 버스트·네이버 장애로 오판할 수 있어 오판 2회면 하루 뉴스(및 홈 주도 테마 근거)가 통째로 꺼지는 위험. 설계 사용량 ≈15.5K vs 예산 18,750 vs 공식 25,000 이라 실제 소진 가능성은 낮아 이득 대비 비용이 큼. 변경: 사전 skip·`QUOTA_STRIKES_TO_STOP_DAY` 제거, 오늘 strike ≥1 이면 첫 선택 대상을 동시성·429 재시도 없이(`collectStockNews` `retryOnRateLimit=false`) 단독 수집 → 429 면 호출 1건으로 run 중단·strike 1회, 그 외면 `quota probe passed — resuming` 후 정상 진행. 판정 run 즉시 중단은 유지. CLAUDE.md·STACK.md 공식 API 기준 4항 동일 문장 갱신. 게이트: news-sync vitest 139 통과(사이클 테스트 10건 신설)·typecheck·build, `news-sync:24bc6b3` 배포 smoke 11/11, 스케줄러 diff 0, 13:42·13:45 예약 실행 성공. 미관측: 오늘 strike 0 이라 라이브 탐침 경로 미발동(테스트로만 검증), 429 코드 매핑 여전히 가정 | 2026-09-15 | 24bc6b3 | [260915-il4-news-sync-quota-probe-recovery](./quick/260915-il4-news-sync-quota-probe-recovery/) |
| 260915-doz | **radar-gw WireGuard — alex-mac(10.20.0.2) 만 KB 121(10.41.1.121) 의 tcp 22·9100 에 영구 허용** — gh-trade 세션 요청(120·121 하드웨어 비교 중 121 이 WireGuard 로 안 닿음). `infra/relay/startup.sh` 섹션 8 의 두 층(nft `inet wgfwd` forward 규칙 + wg0.conf PostUp/PostDown iptables `DOCKER-USER` 멱등 쌍)에 출발지 10.20.0.2 로 묶은 121 규칙 추가 — 120 규칙 뒤·established 앞, 다른 피어(junysim-win1~4)는 120 전용 유지. 라이브 규칙은 같은 날 사용자가 런타임으로 먼저 넣어 둠, 이번에 파일·VM `startup-script` 메타데이터에 영구 반영(재부팅·wg-quick 재시작 없음). 맥 `KB-DMA.conf` AllowedIPs 에 121/32 추가는 저장소 밖(사용자). 문서: README 섹션·dma-tunnel-guide.md·setup-relay-iam.sh 주석. 검증: bash -n · VM `nft -c` · 메타데이터 == 파일 · 라이브 nft/iptables == 파일 의도. 후속 주의: `install-vpn-menubar.sh` 재실행 시 맥 AllowedIPs 가 /32 하나로 덮여 121 이 빠짐. **push 안 함** | 2026-09-15 | 5625273·0b2b8a3 | [260915-doz-radar-gw-wireguard-alex-mac-kb-121-22-91](./quick/260915-doz-radar-gw-wireguard-alex-mac-kb-121-22-91/) |
...
| 260917-g45 | **relay 업타임 경보 싱가포르 AND 조건 + wg-probe `hs_stale` 기준 보정** — 09-17 03:39 KST 부터 해외 지점(미국·유럽·브라질)만 10초 시간초과로 실패해 경보가 반복됐으나 Caddy 수신 `/healthz` 는 전부 200·싱가포르 무실패(요청이 VM 에 도달 못 한 경로 문제). 조건을 「6지점 평균 <0.9 **AND** `apac-singapore` <0.9」로 변경 — 11일 백테스트에서 실장애 5건 전부 유지·해외 경로 오탐 5건 전부 제거, 정책 update 만 반영. wg-probe `hs_stale` 은 앵커를 「만료 뒤 첫 이동」+3초 유예로 바꿔 휴면 피어 오탐 2형(만료 전 이동 잔상·1초 깨어남) 제거, self-check D6 신설(뮤테이션 확인), 장중 파일 설치+재기동·메타데이터 키만 갱신 | 2026-09-17 | (본 커밋) | [260917-g45-relay-and-wg-probe-hs-stale](./quick/260917-g45-relay-and-wg-probe-hs-stale/) |
| 260920-pik | **netcut 측정기 4종 저장소 박제 + 「61분 주기 국내 방향 단절」 사건(2026-09-17~18) 기록** — `infra/relay/netcut-probe.sh`·`netcut-daily.sh`·`netcut-daily.service`·`netcut-daily.timer` 를 VM 배치본과 sha256 동일하게 박제(4건 전부 첫 작성에서 일치, VM 변경 0·`gcloud` 0회)하고 README 에 §외부 경로 단절 판정 — netcut 신설. 측정 5축(메타데이터 HTTP·8.8.8.8·KT·LG U+·KB via tun0·ens4 카운터/CPU steal, 1초 주기). 사건: 9/17 14:08:03 부터 61분(−1초~+4초) 주기 12회, 회차별 2~15초, 대상이 KT/LGU 로 회차마다 바뀜. 매 회차 8.8.8.8·메타데이터 무응답 0(9/18 측정분 8.8.8.8 17,976회 중 0) → 판정 「GCP 서울 ↔ 국내 ISP 피어링 구간」. WireGuard 무혐의(wg0 를 안 타는 ICMP·openconnect·relay DMA TCP 도 같은 초에 끊김). 호스트 원인 배제: 9/17 21:57 stop→start 후에도 주기·분초 위치 불변. 9/18 17:11 마지막으로 소멸(9/19 개별 1건·9/20 0건). 운영: `netcut-daily.timer` enabled(평일 08:00 KST, 재부팅 생존) 이나 **메타데이터 startup.sh 미등록 → VM 재생성 시 소멸**. 네트워크 등급 Premium 유지, Standard 전환은 비용 별건으로 남김. 미실행: `systemd-analyze verify`(로컬 systemd 없음, sha 동일성으로 대체) · README §메모리 예산 행(RSS 미실측) | 2026-09-20 | ce97f40 | [260920-pik-netcut-61](./quick/260920-pik-netcut-61/) |
