...
| TRADE-03 | Phase 16 | Pending — **잔여 1건: WinForms ↔ 웹 「한 세션」 동기화 실측.** 2026-09-09 갭 클로징 **3라운드** 종결(16-36~16-46) 후 재판정. **Pending 사유가 갱신됐다** — 이전 판의 사유였던 「프로덕션에서 이 경로가 한 프레임도 나른 적이 없다(`everReadyCount:0`)」는 **더 이상 참이 아니다.** 실 게이트웨이(`10.41.1.120:9100`) 결선 후 `https://dma.jx1.io/healthz` 가 **200** `{"status":"ok","vpn":true,"dma":true,"version":"a1f4ed6","sessionCount":1,"everReadyCount":1,"stalledCount":0}` 이다 — Ready 에 도달한 DMA 세션이 프로덕션에 실재한다. 코드 층위는 1라운드 14건 · 2라운드 19건에 더해 **3라운드 17건**(R2-CR-01 무장 가드 우회 · R2-CR-02 영구 503 · R2-CR-03 계좌번호 로그 유출 · R2-WR-01~07 · R2-IN-01~05 · 갭 4 사이드바 ISIN · 갭 5 배포 DMA_HOST 보존)이 전부 닫혔고 전량 게이트가 green 이다(`pnpm -r test` **2,044 pass** / 191 파일 · relay 397 · webapp 680 · e2e 126/9/0 · `pnpm build` exit 0 · `typecheck` · relay `typecheck:tests` 전부 exit 0). relay 는 커밋 **`a1f4ed6`** 로 재배포됐고 webapp 은 프로덕션 청크 **내용 대조**로 반영이 확인됐다(`2422-ff4cf64d654c9829.js` 안 `useIsinLabels` 의 `limitChasers` 순회). **그럼에도 Pending 인 이유는 단 하나다** — `everReadyCount: 1` 은 「relay 가 게이트웨이에 붙어 DMA 세션이 Ready 상태에 도달했다」까지만 말하며, **「WinForms 와 전략·체결·미체결이 즉시 공유된다」는 뜻이 아니다.** 후자는 relay 가 같은 DMA 세션을 쓴다는 구조에서 파생될 것으로 **기대되는** 결과이지 관측된 사실이 아니다. 그 실측(`16-VERIFICATION.md` §Human Verification #1)은 human-only 이며 D-27 상 사용자 명시 지시가 필요하다. 실주문 검증도 하지 않았다. 별도 열린 항목: smoke `INV-9` 는 `SMOKE_AUTH_TOKEN` 부재로 **프로브 본체가 한 줄도 실행되지 않았다**(「돌렸는데 SKIP」이 아니다) — 16-21 재작성 이후 프로덕션 첫 실행 미수행. **RELAY-02 와 같은 기준을 유지한다**(사용자 결정 2026-09-09) — 두 요구사항의 판정 기준을 갈라 놓으면 다음 사람이 어느 쪽을 믿을지 모른다. 진전은 잔여가 **1건으로 좁혀졌다**는 것이다. 근거 정본: `16-VALIDATION.md` §Gap Closure 3라운드 (16-36 ~ 16-46) · §Deployment Verification (16-46 Task 3) |
