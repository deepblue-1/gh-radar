...
| TRADE-03 | Phase 16 | Pending — 잔여: **프로덕션에서 이 경로가 한 프레임도 나른 적이 없다.** 2026-09-09 2라운드 갭 클로징 종결(16-35) 후 재판정. **코드 층위는 전부 닫혔다** — 1라운드 14건(gap 1·2 포함)에 더해 2라운드 19건(GC-CR-01 통보 하드 필터 · GC-CR-02 수동 통보 조회 경유 · GC-CR-03 await TOCTOU 가드 · GC-WR-01~12 · GC-IN-01~04)이 닫혔고, 전량 스위트가 green 이다(`pnpm -r test` 2,012 pass / 190 파일 · relay 373 · e2e 126/9/0 · `pnpm build` exit 0). relay 는 커밋 `c8aa7ae` 로 재배포됐다. **그럼에도 Pending 인 이유는 프로덕션 실측이다** — `https://dma.jx1.io/healthz` 가 `{"version":"c8aa7ae","sessionCount":2,"everReadyCount":0,"stalledCount":2}` = **503 degraded** 이다(2026-09-09 16:17 KST). `everReadyCount: 0` 은 **Ready 에 도달한 DMA 세션이 프로덕션에 한 건도 없었다**는 뜻이고, `stalledCount: 2` 는 그 상태가 5분 유예를 넘겨 지속 중임을 뜻한다. 즉 전략 중계·팬아웃·주문 5초 상관이 실서버에서 **한 번도 실행된 적이 없다**. `DMA_HOST` 는 D-27 상 로컬 mock(`127.0.0.1:9100`)이며 그 mock 은 VM 에 떠 있지 않고, 실서버 결선은 **사용자의 명시 지시가 있을 때만** 한다. WinForms ↔ 웹 세션 공유(16-VERIFICATION §Human Verification Required #1)도 미실행이다. **mock·단위 검증만으로 Complete 로 올리지 않는다** — RELAY-02 와 같은 기준이다. 근거 정본: `16-VALIDATION.md` §Gap Closure 2라운드 (16-27 ~ 16-35) · §Deployment Verification (16-35 Task 2) |
