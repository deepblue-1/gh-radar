...
- [x] **TRADE-06**: 돌파감지 목록 — `/trading` 상단 돌파 스트립(한 줄 칩 + 「더보기」 전 열 표, 거래소 미표시). 76 upsert·78 전량 교체·서버 정렬(새 돌파 상단·재돌파 자리 유지)을 재해석하지 않음, 임계−2%p 이탈 즉시 삭제(목록 종목 시세 구독), 알림음 종목당 하루 1회(상태줄 토글·기본 꺼짐), 「오늘 울린/지운 종목」 localStorage KST 날짜 키, 30초 연노랑 강조 + 수동 ✕, 칩/행 클릭 → 카드 추가(KRX 시작) — Phase 18
...
- [x] **TRADE-08**: NXT VI 설정 — `/trading` 상단 VI 설정 2줄(KRX/NXT: on/off=run · 상승률 % · 금액 만원 · 가동중 배지 · 더티 시 「수정」, `vi.set` 에 줄의 거래소 송신, `VI_EDIT_EXCHANGE` 상수 폐기) + VI 발동 스트립(칩 + 「더보기」 표, 양 거래소 한 목록, 확인 체크는 표에서만·기존 `vi-order-list` 규칙·110초 진행바)이 기존 `/trading/vi` 화면을 대체 — Phase 18
...
| TRADE-08 | Phase 18 | Complete |
| TRADE-09 | Phase 18 | Complete |
