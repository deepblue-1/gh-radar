...
- [x] **RELAY-02**: 지정가 보통 신규 매수/매도 + 취소 주문 릴레이 — 브라우저 → Cloud Run `POST /api/orders`(requireAuth + allowlist + 형식 검사, 금액·수량 한도 없음) → Direct VPC Egress → VM relay 내부 HTTP(공유 비밀 헤더) → `DirectOrderReq(2)`, 첫 `OrderResp(51)`(접수 "A"/거부 "R") ≤5초 응답, 체결("E")·취소확인("C")은 주문자 wss 푸시, `dma_orders` 기록 + 오늘 주문 목록 복원, `stocks.isin` 코드↔ISIN 매핑, 활성 세션 없으면 409 — Phase 15
...
- [x] **TRADE-03**: relay 전략 중계 + 주문 wss 이관 — 인바운드 `lc.set`/`vi.set`/`vi.confirm`/`strategies.disable`/`order.new`/`order.cancel`, DMA Ready 시 24/21/34 프리페치 + 세션 전략 캐시, auth 직후 전략 스냅샷 팬아웃, 56/60/61/64/65/72/73 파싱·팬아웃, `DirectOrderReq(2)` 5초 상관 이관 + `dma_orders` insert/update 를 relay 가 전담(origin manual/limit_chaser/vi), server `POST /api/orders` 제거(GET 유지) — Phase 16
- [x] **TRADE-04**: gh-trade 프로토콜 재동기화 + 신규 푸시 중계 + 기존 화면 보정 — `relay/src/generated` 를 gh-trade HEAD(`59f7513e` 배포분 이상) 스키마로 재생성, MsgType 36/37/38(C→S)·76/77/78(S→C) 등록, 76/77/78 파싱·세션 캐시·인증 스냅샷·브라우저 프레임(`rate.cross`/`rate.cross.snap`/`queued.window`, UI 없음), `unknown-msg-type` 드롭 0. 기존 화면: 체결테이프 `bs_code` 실값 색(없을 때만 추정 폴백), 호가 종목정보 10칸 `krx_close_price>0 → '종가'`, 미체결 `queued_status/pending_status/board/pending_cancel_sent` 표식(`매수Q/P//종가` 헬퍼 하나)·`pending_cancel_sent` 회색+취소 숨김, 주문통보 `request_kind/requester/board` 행위 단어·수동·시간외종가(`message` 미파싱), 자동주문 통보 3초 창 묶기, `ServerMessage.source` `[상따]/[VI]` 배지, VI `exchange` 축(21 KRX/NXT 2회 조회·거래소별 캐시·`vi.set` exchange·거래소 열) — Phase 17
- [x] **TRADE-05**: 상따 3단계 래치 LED 3종 + 수동 점등 — `/trading/limit-chaser` 상태줄에 매수·매도·취소 LED(회색 무장 아님 / 주황 잠복 / 초록 래치 ON, C# `LimitChaserForm` 규칙 동일: 취소 무장 = `cancel_qty_enabled || cancel_trade_enabled`, 매수는 `buy_watch_side "1"` 일 때만 3단계·매도잔량 기준은 초록 2단계·클릭 불가), 텍스트 라벨·툴팁 동반, 클릭 토글 = `{t:"lc.arm", key, latch}` → `ArmSellLatchReq 36 / ArmCancelLatchReq 37 / ArmBuyLatchReq 38`(`get_strategy_req.key` = 전략 키), 확인 다이얼로그 없음, 판정 근거는 서버 에코 스냅샷, 결과는 60 에코 색·실패는 54 문구, 전략 로그에 취소/매수 래치 전이 4종 — Phase 17
- [ ] **TRADE-06**: 돌파감지 목록 — `/trading` 상단 돌파 스트립(한 줄 칩 + 「더보기」 전 열 표, 거래소 미표시). 76 upsert·78 전량 교체·서버 정렬(새 돌파 상단·재돌파 자리 유지)을 재해석하지 않음, 임계−2%p 이탈 즉시 삭제(목록 종목 시세 구독), 알림음 종목당 하루 1회(상태줄 토글·기본 꺼짐), 「오늘 울린/지운 종목」 localStorage KST 날짜 키, 30초 연노랑 강조 + 수동 ✕, 칩/행 클릭 → 카드 추가(KRX 시작) — Phase 18
- [ ] **TRADE-07**: 예약/장전/시간외종가 발주 + 수동주문(신규·정정·취소) — 77 힌트는 라벨·입력 전환에만(`undefined`=모름=전부 false, 벽시계 판정 금지, 주문 차단 금지): `open`∧KRX → 「예약매수/예약매도」+조각 수(기본 5·상한 `maxPieces`), `preopenOpen`/`nxtPreopenOpen` → 예약 라벨+조각 숨김+「예약: 증권사 보관 후 09:00 처리」(NXT 08:00). 주문 프레임에 `pieceCount`·`krxSession` 추가(shared·webapp 번역기·relay 빌더). 시간외종가(G2/G3)는 종목상세 호가 탭 수동주문 폼의 주문유형 콤보에만(KRX∧창 열림 시 활성·가격 잠금·참고 종가·price 0). 수동주문 폼 = 가격·수량·(조각)·「매수·매도·정정·취소」, 정정/취소는 미체결 행 선택 시 활성, 정정(`order_type "M"`)은 relay D-21 게이트 해제. 배치는 적응형(컨테이너 <700 「매수|매도|수동」 탭, ≥700 「수동주문」 덮기) — Phase 18
- [ ] **TRADE-08**: NXT VI 설정 — `/trading` 상단 VI 설정 2줄(KRX/NXT: on/off=run · 상승률 % · 금액 만원 · 가동중 배지 · 더티 시 「수정」, `vi.set` 에 줄의 거래소 송신, `VI_EDIT_EXCHANGE` 상수 폐기) + VI 발동 스트립(칩 + 「더보기」 표, 양 거래소 한 목록, 확인 체크는 표에서만·기존 `vi-order-list` 규칙·110초 진행바)이 기존 `/trading/vi` 화면을 대체 — Phase 18
- [ ] **TRADE-09**: 통합 트레이딩 작업대 `/trading` — 상태줄(1·2·3단 세그먼트, 폰 밴드 숨김) → VI 설정 2줄 → VI 발동 스트립 → 돌파 스트립 → 종목 추가 검색 → 다종목 상따 카드 격자(펼친 카드 우선 + 접힌 카드 한 칸 스택 · 헤더: 종목명·코드·KRX|NXT 토글(등록 전만)·현재가·LED 3칩·ⓘ·캐럿·✕ · 종목정보 10칸 · 본문 좌 호가+체결 | 우 옵션 4그룹(§2.2b 4밴드를 카드 폭 기준으로, 스위치는 그룹 제목줄, 섹션 라벨 없음)) → 하단 공용 미체결·잔고·전략 로그. 종목상세 호가 탭 = 같은 카드 본문 문법(+거래소 세그먼트·주문유형 콤보), 카드 ⓘ 종목정보 팝업(차트|종목정보|뉴스·토론). 기존 `/trading/limit-chaser/*`·`/trading/vi` 제거 + `/trading(?focus=)` 리다이렉트, 사이드바 「트레이딩」 제목 링크 + 3단(KRX VI·NXT VI·상따 종목). 정본 목업 `18-workbench-mockup.html`·`18-orderbook-tab-mockup.html` — Phase 18
...
| ~~모바일 앱~~ | **2026-09-25 정정 — 범위 안으로 이동:** Phase 21 이 웹을 감싸는 Capacitor Remote-URL 셸 앱(GH Trade)을 v1 **MOBILE-01** 로 만든다. 푸시 알림·딥링크·스토어 **정식 출시·심사**는 여전히 범위 밖(테스트 배포는 MOBILE-02 — 2026-09-27 정정) |
...
| RELAY-02 | Phase 15 | Complete — 2026-09-10 장중 실측으로 §8 잔여 2건이 닫혔다. ① **취소 왕복**: `order.cancel` → 통보 `C` → `cancelled` 1건을 relay 와이어 로그에서 관측(신규 접수 `A` 2건 동반) · **첫 통보 지연 21~32 ms** — D-22 의 5초 상한 대비 약 1/200. 단 §8 의 median 77 ms 는 `dma_orders` 의 **DB 측 Δ 근사**였고 오늘 값은 **와이어 로그의 첫 통보 지연**이다 — **다른 계측이므로 같은 축에서 비교하지 않는다.** ② **오늘 주문 목록 복원(D-24)**: 사용자가 프로덕션 `/me` 에서 3건(접수 2 · 취소 1) 표시를 확인. 이 잔여는 **검증 공백이 아니라 기능 공백**이었다 — 서버 라우트는 살아 있었으나 `webapp/src` 에 **호출자가 0건**이었고(16-16 이 주문 **접수**만 wss 로 옮겼다), quick-260910-jce 가 `/me` 에 배선해 닫고 quick-260910-kql 이 종목명을 병기했다. **정의문 대조:** 위 정의문의 `POST /api/orders` 는 Phase 16(16-16)이 **wss 단일 경로로 의도적으로 대체**했고 REST 는 `GET`(복원)만 남았다 — **결손이 아니라 설계 갱신**이며, 판정은 경로 문자열이 아니라 조항의 기능(주문 릴레이 · 통보 반영 · 오늘 주문 복원)이 성립하는가로 한다. 계좌번호는 프로덕션 로그에 **말미가 가려진 마스킹 형태**로만 남는다(R2-CR-03 동작). 근거 정본: `15-LIVE-VERIFICATION.md` §9 · quick-260910-jce SUMMARY |
...
| TRADE-03 | Phase 16 | Complete — 잔여 1건이던 **WinForms ↔ 웹 「한 세션」 동기화**를 2026-09-10 장중에 **사용자가 양방향으로 직접 관찰**했다(웹 `/trading/limit-chaser` 조작 → WinForms 반영 · WinForms 조작 → 웹 반영). **증거 형태:** 이 항목은 `16-VERIFICATION.md` §Human Verification Required #1 이 애초에 **human-only** 로 지정한 것이라 사용자 관찰이 **의도된 증거 형태**다. **로그 코러버레이션과 그 한계:** relay 로그에 `[HUB] 상따 에코 수신` 6건(`crud` C×4 · D×2, 캐시 갱신 동반)이 남아 **같은 DMA 세션에서 전략 에코를 받는다는 메커니즘**을 뒷받침한다. 다만 relay 는 **성공한 인바운드 전략 요청을 로그로 남기지 않으므로**(거부만 기록) **각 에코의 발신 방향을 로그만으로는 가릴 수 없다.** 방향 판정의 근거는 사용자 관찰이다. 배포 상태: relay `11072e4`(장중, 사용자 승인) · `/healthz` `everReadyCount` 2 · `stalledCount` 0 · `DMA_HOST` 무주입 보존(Phase 16 갭 5 동작). **별도 열린 항목 승계:** smoke `INV-9` 는 `SMOKE_AUTH_TOKEN` 부재로 16-21 재작성 이후 **프로덕션 첫 실행 미수행**이다 — 이것은 TRADE-03 조항의 결손이 아니라 **스모크 프로브의 미실행**이므로 판정을 막지 않는다. 근거 정본: `16-VALIDATION.md` §Gap Closure 3라운드 · `15-LIVE-VERIFICATION.md` §9 |
...
| TRADE-04 | Phase 17 | **Complete** (2026-09-21 장중 실기 관측) — 코드 층위는 17-01~17-10 이 닫았고 자동 게이트가 green 이다(루트 `pnpm typecheck` exit 0 · relay **467** · webapp **1008** · shared **108** · Playwright **135 pass · 0 fail** · `sync-relay-schema.sh --check` 차이 **0**). 조항별 근거는 종전과 동일(재동기화·MsgType = 17-01 D1·D2 / 76·77·78 = 17-03 D1~D4 / 체결테이프·호가 = 17-02 D1·D2 + 17-08 D1~D4 / 미체결 = 17-09 D1~D3 / 주문통보·3초 묶기 = 17-02 D4 + 17-10 D1~D4 / 배지 = 17-08 D5 + 17-06 D5 + 17-11 / VI 거래소 축 = 17-05 + 17-06). **Pending 을 풀어 준 실기 관측 (2026-09-21, 프로덕션 `ef1499a`, 장중):** ① **`unknown-msg-type` 드롭 0 실증** — 프로덕션 relay 컨테이너 로그 6시간 구간 527행에서 `unknown-msg-type` **0건**. ② **신규 MsgType 78 이 실서버에서 실제로 도착·처리됐다** — `[HUB] 돌파 집합 스냅샷 수신 — 전량 교체` **6건**. 화이트리스트 등록·hub `case`·팬아웃 경로가 서버 HEAD 상대로 종단 동작함을 뜻한다(mock 이 아니다). ③ `[HUB] 상따 에코 수신` **36건** — 60 에코 경로 정상. **정직 기록:** 76(`RateCrossAlert`)·77(`QueuedWindowState`)은 이 구간에 **발화하지 않아 실수신을 보지 못했다**(둘 다 info 로그가 없어 로그로는 판별 불가). 다만 드롭되었다면 `unknown-msg-type` 이 섰을 텐데 0건이고, 78 이 같은 등록 경로로 실동작했으며, 세 종 모두 17-03 의 mock 게이트웨이 테스트가 덮는다. **이날 이 표면에서 프로덕션 결함 1건을 발견·수정했다** — `/limit-chaser/new` 에서 「미반영」·「수정」 버튼 영구 고착(debug `lc-unacked-stuck-new-route`, 수정 `b31435b` 배포 완료). |
| TRADE-05 | Phase 17 | **Complete** (2026-09-21 장중 실기 관측) — LED 3종의 색·단계·클릭 규칙과 `lc.arm` 전송이 코드 층위에서 닫혔고(판정 순수함수 19케이스 = 17-07 D2·D3 / 라벨·접근성 = 17-07 D4 / relay `lc.arm` → 36·37·38 왕복 가드 19케이스 = 17-04 D1~D4 `ws-latch.test.ts` / 상태줄 결선·전략 로그 전이 4종 = 17-11 D1~D4), D-22 dev 화면 확인도 2026-09-20 스크린샷 8장으로 사용자 승인을 받았다. **Pending 을 풀어 준 실기 관측 (2026-09-21, 프로덕션 `ef1499a`, 장중):** 사용자가 상따 화면에서 래치 LED 클릭 왕복이 **정상 동작함을 직접 확인**했다(「LED 잘돼」). 프로덕션 relay 로그가 같은 구간 `[HUB] 상따 에코 수신` **36건**으로 이를 뒷받침한다 — 클릭이 `lc.arm` → 36/37/38 로 나가고 60 에코가 돌아와 색이 바뀌는 종단 경로가 스텁이 아닌 **실 게이트웨이 상대로** 관측됐다. 이로써 TRADE-04 ①과 공유하던 Xcode 라이선스 게이트 차단 사유가 해소됐다(mock 없이 실서버로 닫음). |
...
- v1 requirements: 53 total (MOBILE-02 added 2026-09-27 with Phase 22 plan-phase — Out of Scope 「~~모바일 앱~~」 행의 「스토어 제출」 을 「스토어 정식 출시·심사」 로 정정, 테스트 배포는 범위 안 → 52→53; MOBILE-01 added 2026-09-25 with Phase 21 plan-phase — Out of Scope 「모바일 앱」 정정 → 51→52; TRADE-06·TRADE-07·TRADE-08·TRADE-09 added 2026-09-21 with Phase 18 discuss-phase 범위 확장 → 47→51; TRADE-04·TRADE-05 added 2026-09-18 with Phase 17 → 45→47; DISC-01.1 added in Phase 08.1; DATA-01 added 2026-05-10 with Phase 9 의미 교체; DATA-02 added 2026-05-13 with Phase 09.1 인서트; NEWS-02·DISC-02 removed 2026-06-08 구 Phase 10(AI Summarization) 삭제; 2026-06-08 SCAN-08 매핑 누락 보강 + 카운트 27→29 정합 정정; THEME-01·THEME-02 added 2026-06-08 with Phase 10(Theme Classification — 삭제된 구 Phase 10 번호 재사용) → 29→31; THEME-03(유저 CRUD)·THEME-04(AI 보강) added 2026-06-09 Phase 10 discuss-phase 스코프 확장 → 31→33; COMV-01 added 2026-06-11 with Phase 11(Co-movement Candidates) → 33→34; LIMIT-01 added 2026-06-26 with Phase 12(상한가 다음날 이력 통계) → 34→35; HOME-01 added 2026-07-01 with Phase 13(홈 급등 테마 AI 분석) → 35→36; CHAT-01 added 2026-07-02 with Phase 14(AI 애널리스트 챗봇) → 36→37; RELAY-01·RELAY-02·RELAY-03 added 2026-09-05 with Phase 15(DMA 중계 서버) → 37→40; TRADE-01·TRADE-02·TRADE-03·NAV-01·MYPAGE-01 added 2026-09-08 with Phase 16(트레이딩 메뉴 — 상따·VI·My page) → 40→45)
- Mapped to phases: 53
- Unmapped: 0 ✓

---
*Requirements defined: 2026-04-10*
*Last updated: 2026-09-08 — Phase 16 (trading-limit-chaser-vi-my-page) plan-phase: TRADE-01/02/03·NAV-01·MYPAGE-01 v1 정의 + Traceability(Pending) 5행 + Coverage 40→45. / quick-260908-scu: Phase 15 RELAY-01/02/03 재판정 — RELAY-01·RELAY-03 Complete, RELAY-02 는 취소(`C`) 왕복·주문 목록 복원 잔여로 Pending 유지(근거 `15-LIVE-VERIFICATION.md` §8). / 2026-09-09 — Phase 16 갭 클로징 종결(16-18~16-26, 14건): TRADE-01·TRADE-02·NAV-01 을 Pending→**Complete** 로 재판정(전량 스위트 green + 프로덕션 `2cb5620` 배포 실측). MYPAGE-01 은 16-19 에서 이미 Complete. **TRADE-03 은 Pending 유지** — 코드 층위(gap 1·2 포함)는 전부 닫혔으나 프로덕션 `/healthz` 가 `everReadyCount:0` 이라 relay 전략·주문 경로가 실서버에서 한 번도 실행된 적이 없다(D-27 상 실서버 결선 미실시). 근거 정본: `16-VALIDATION.md` §Gap Closure · §Deployment Verification (16-26). / 2026-09-09 — Phase 16 갭 클로징 **2라운드** 종결(16-27~16-35, GC- 19건): TRADE-01·TRADE-02·NAV-01·MYPAGE-01 은 Complete 유지(체크박스와 Traceability 가 5개 ID 전부에서 일치함을 재확인). **TRADE-03 은 Pending 유지** — 코드 19건이 닫히고 relay 가 `c8aa7ae` 로 재배포됐으나 프로덕션 `/healthz` 가 `everReadyCount:0` · `stalledCount:2` · 503 degraded 로, DMA 경로가 실서버에서 한 프레임도 나른 적이 없다(D-27 상 실서버 결선 미실시). 근거 정본: `16-VALIDATION.md` §Gap Closure 2라운드 · §Deployment Verification (16-35). / 2026-09-09 — Phase 16 갭 클로징 **3라운드** 종결(16-36~16-46, 17건: R2-CR 3 · R2-WR 7 · R2-IN 5 · 갭 4 · 갭 5): TRADE-01·TRADE-02·NAV-01·MYPAGE-01 은 **Complete 유지**(체크박스와 Traceability 가 5개 ID 전부에서 일치함을 재확인 — TRADE-01 `[x]` · TRADE-02 `[x]` · TRADE-03 `[ ]` · NAV-01 `[x]` · MYPAGE-01 `[x]`). **TRADE-03 은 Pending 유지 — 다만 사유를 갱신했다.** 옛 사유(「Ready 도달 이력 0」)는 실 게이트웨이 결선으로 해소됐고(`/healthz` 200 · `everReadyCount:1`), **잔여는 「WinForms ↔ 웹 한 세션 동기화 미실측」 1건**이다. relay 는 `a1f4ed6` 로 무주입 배포(갭 5 실증) · webapp 은 청크 내용 대조로 반영 확인 · **server 는 「diff 는 타입 전용 + 소비처 0건」 근거로 의도적으로 건너뛰었다**(배포 누락 아님). 근거 정본: `16-VALIDATION.md` §Gap Closure 3라운드 · §Deployment Verification (16-46). / 2026-09-10 — quick-260910-ogq: 장중 실계좌 실측(취소 `C` 왕복 · `/me` 오늘 주문 복원 사용자 확인 · WinForms↔웹 양방향 사용자 관찰 · relay `11072e4` 배포)을 반영해 **RELAY-02 · TRADE-03 을 Pending → Complete** 로 재판정. 정의부 체크박스와 Traceability 대칭 확인(요구사항 총계 45 · Coverage 무변경). SC-4(마지막 wss 종료 5분 뒤 세션 종료)는 **⚠ 유지** — 오늘도 관측하지 않았다. 근거 정본: `15-LIVE-VERIFICATION.md` §9. / 2026-09-20 — Phase 17 (17-12) 종결 판정: **TRADE-04 · TRADE-05 는 Pending 유지**. 정의부 체크박스 `[ ]` 와 Traceability 가 양쪽 모두 Pending 으로 일치한다. 코드 층위는 17-01~17-11 이 전부 닫았고 자동 게이트가 green(루트 typecheck · relay 467 · webapp 998 · shared 108 · Playwright 135 pass/0 fail · 재동기화 --check 차이 0)이지만, **D-25 mock 게이트웨이 실기 검증이 Xcode 27.0 라이선스 미동의(`sudo xcodebuild -license`)로 수행 불가**였고 D-26 배포도 사용자 승인 대기다. 요구사항 총계 47 · Coverage 무변경. 근거 정본: `17-12-SUMMARY.md`. / 2026-09-21 — Phase 18 discuss-phase: 사용자 결정으로 범위를 「화면 3개 신설」에서 「상따+VI 통합 트레이딩 작업대 `/trading` 재설계」로 확장. TRADE-06/07/08 v1 정의 + TRADE-09(통합 작업대) 신설, Traceability Pending 4행, Coverage 47→51. 근거 정본: `18-CONTEXT.md`. / 2026-09-21 — **TRADE-04 · TRADE-05 를 Pending → Complete 로 재판정.** 근거는 프로덕션 `ef1499a` 장중 실기 관측이다: 사용자 확인 래치 LED 클릭 왕복 정상 + relay 로그 `상따 에코` 36건 · `돌파 집합 스냅샷 수신`(MsgType 78) 6건 · `unknown-msg-type` **0건**(6시간 527행). D-25 는 mock 이 아니라 **실서버로** 닫혔다(Xcode 라이선스 게이트는 우회한 것이 아니라 불필요해졌다). 정의부 체크박스와 Traceability 대칭 확인. 요구사항 총계 51 · Coverage 무변경. 같은 날 이 표면에서 프로덕션 결함 1건(`lc-unacked-stuck-new-route`)을 발견·수정·배포했다(`b31435b`). / 2026-09-22 — Phase 18 갭 클로징(18-14~18-23, 10건: CR-01·CR-02·돌파 최신순·WR-01~07): TRADE-06~09 를 Gaps Found → **Pending**(재검증 대기)으로 되돌렸다. Complete 판정은 재검증 몫이다. 10건 모두 닫혔고(재현 안 됨 0 · 보류 0) 전량 게이트 green(build_command `error TS` 0 · relay 516 · webapp 1383 · Playwright 141 pass/0 fail/9 skip). CR-01 원격 CHECK 는 18-18 에서 사용자 `apply` 로 반영됐다. **relay 는 미배포** — 배포 순서 DB(완료) → relay → 검증 → webapp push. 정의부 체크박스(TRADE-06~09 `[ ]`)와 Traceability(Pending 4행) 대칭 확인. 요구사항 총계 51 · Coverage 무변경. 근거 정본: `18-VALIDATION.md` §Gap Closure. / 2026-09-22 — Phase 18 갭 클로징 **R3**(18-24~18-32, 18-REVIEW-R2 11건 + deferred relay 콜드 세션 수정): TRADE-06~09 는 **Pending 유지**(재검증 대기 · relay 미배포). 12행(GC-CR-01 로컬·원격 · GC-WR-01~04 · GC-IN-01~06(IN-02 에 deferred relay 수정 포함)) 모두 닫힘 — 재현 안 됨 0 · 보류 0, 코드 수정 행은 수정 전 RED 를 재현한 뒤 GREEN. GC-CR-01 원격 CHECK 는 18-29 에서 사용자 `apply` 로 **반영**됐다(위반 행 0 사전 확인 · `migration list --linked` `20260922180000` Local=Remote · 덤프 diff CHECK 1줄) — R2 의 「DB 가 가격 0 신규·정정을 거부한다」 전제는 3값 논리로 사실이 아니었음을 18-VALIDATION §Gap Closure R3 정정 기록에 남겼다. 전량 게이트 green(build_command `error TS` 0 · relay 523 · webapp 1454 pass/1 skip · Playwright 142 pass/0 fail/9 skip · GC1~GC5 pass · 로컬 DB 회귀 12/12). 배포 순서 DB(18-18 · 18-29 완료) → relay(R2 18-14·17·19 + R3 18-25·18-26) → 검증 → webapp push — 18-26 의 webapp 변경은 relay 18-26 배포 뒤에만 안전하다. Manual-Only UAT 인계 6 → 9항목. 정의부 체크박스(TRADE-06~09 `[ ]`)와 Traceability(Pending 4행) 대칭 확인. 요구사항 총계 51 · Coverage 무변경. 근거 정본: `18-VALIDATION.md` §Gap Closure R3. / 2026-09-22 — Phase 18 갭 클로징 **R4**(18-33~18-36, 18-REVIEW-R3 7건 + 18-VERIFICATION-R3 escalation 2건): TRADE-06~09 는 **Pending 유지**(재검증 대기 · relay 미배포). 7행(R3-WR-01 · R3-WR-02 · R3-IN-01~05) 모두 닫힘 — 재현 안 됨 0 · 보류 0. R3-WR-01 은 수정 전 ws ㊹ ㊺ RED 를 재현한 뒤 relay 조건부 UPDATE(상태 단조성, 마이그레이션 없음)로, R3-WR-02 는 수정 전 e2e GC6 RED 를 재현한 뒤 결과 모름 잠금을 `RelayProvider`(앱 수명)로 올려 닫았다. R3-IN-03 은 RED 없는 정합성 결함이라 불변식 회귀로 고정했고, R3-IN-04 의 중립 표기와 R3-IN-05 의 계약 버전 handshake 는 deferred-items 로 이연했다. 사용자 결정 2건(2026-09-22 — ① 결과 모름 잠금 앱 수명 · 해제는 로그아웃 · 새로고침뿐 · 호가 탭 공유 ② 잠금 등록은 신규 · 정정만)이 18-30 의 페이지 수명 설계를 대체했다. escalation 2건(UAT-R3 #11 · #12)은 코드로 닫혔고, #11 의 실 게이트웨이 통보 순서 관측만 Manual-Only 로 남는다(UAT 인계 9 → 10항목). 전량 게이트 green(build_command `error TS` 0 · relay 534 · webapp 1472 pass/1 skip · Playwright 143 pass/0 fail/9 skip · GC1~GC6 pass). R4 DB 변경 0(`fa95489..HEAD` `supabase/` diff 없음 — pgTAP 러너 · `migration list` 미실행). 배포 순서 DB(18-18 · 18-29 완료 · R4 변경 0) → relay(R2 18-14·17·19 + R3 18-25·18-26 + R4 18-33) → 검증 → webapp push — 18-26 결합이 그대로 push 를 막고, R4 webapp(18-34 · 18-35)은 새 결합이 없다. 정의부 체크박스(TRADE-06~09 `[ ]`)와 Traceability(Pending 4행) 대칭 확인. 요구사항 총계 51 · Coverage 무변경. 근거 정본: `18-VALIDATION.md` §Gap Closure R4. / 2026-09-27 — Phase 22 (gh-trade-ios-testflight-android-play) plan-phase: MOBILE-02(테스트 배포 — TestFlight 내부 · Play 내부 테스트) v1 정의 + Traceability(Pending) 1행 + Coverage 52→53 · Out of Scope 「~~모바일 앱~~」 행 「스토어 제출」 → 「스토어 정식 출시·심사」 정정.*
