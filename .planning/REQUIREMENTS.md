...
| TRADE-04 | Phase 17 | **Pending** — 코드 층위는 17-01~17-10 이 전부 닫았고 자동 게이트가 green 이다(2026-09-20 재실행: 루트 `pnpm typecheck` exit 0 · relay **467 passed / 19파일** · webapp **998 passed · 1 skipped / 73파일** · shared **108 passed / 9파일** · Playwright **135 passed · 9 skipped · 0 failed** · gh-trade `sync-relay-schema.sh --check` 차이 **0**). 조항별 근거: 재동기화·MsgType 등록 = 17-01-SUMMARY D1·D2 / 76·77·78 파싱·세션 캐시·인증 스냅샷·드롭 0 = 17-03-SUMMARY D1~D4(`rate-cross.test.ts` ④ 드롭 0 게이트) / 체결테이프 `bs`·호가 `kc` = 17-02-SUMMARY D1·D2 + 17-08-SUMMARY D1~D4 / 미체결 표식·취소보관 = 17-09-SUMMARY D1~D3 / 주문통보 3필드·행위 단어·3초 묶기 = 17-02-SUMMARY D4 + 17-10-SUMMARY D1~D4 / `[상따]`·`[VI]` 배지 = 17-08-SUMMARY D5 + 17-06-SUMMARY D5 + 17-11 / VI 거래소 축 = 17-05-SUMMARY D1~D4 + 17-06-SUMMARY D1~D4. **Complete 로 올리지 못하는 이유(2건, 둘 다 미관측이지 반증 아님):** ① **D-25 mock 게이트웨이 실기 검증 미수행** — 유일하게 실행 가능한 gh-trade mac 빌드가 `server/build/stock-dma-server`(2026-09-13)인데 HEAD 재빌드가 **Xcode 27.0 라이선스 미동의**(`sudo xcodebuild -license`, 사람만 가능)로 막혔다. 이 구형 바이너리는 바이트 스캔 결과 `RateCrossSnapshot`(78) 0건 · `ArmSell/Cancel/BuyLatchReq`(36/37/38) 0건 · `krx_close_price` 0건 · `request_kind`/`requester` 0건이라, 띄워도 「76/77/78 드롭 0」은 **아무것도 오지 않아서** 참이 되고 래치 왕복은 관측 자체가 불가능하다 — 거짓 양성이 되므로 **일부러 돌리지 않았다**(T-17-46). ② **미배포** — D-26 배포가 사용자 승인 대기 중이라 실서버 장중 관찰이 0회다. 잔여 관찰 항목은 17-12-SUMMARY §사용자 장중 관찰 체크리스트. |
| TRADE-05 | Phase 17 | **Pending** — LED 3종의 색·단계·클릭 가능 규칙과 `lc.arm` 전송은 코드 층위에서 전부 닫혔다. 근거: 판정 순수함수 `latchLedStateOf` 규칙표 19케이스 = 17-07-SUMMARY D2·D3(매도잔량 기준 매수 초록·클릭 불가 T-17-23, 취소 무장이 `cancelQtyTrackEnabled` 를 읽지 않음 T-17-24 봉인) / 라벨·접근성 OFF·대기·감시 + `aria-pressed` = 17-07-SUMMARY D4 / relay `lc.arm` → 36/37/38 Envelope 왕복과 가드 19케이스 = 17-04-SUMMARY D1~D4(`ws-latch.test.ts`) / 상태줄 결선·무반응 조건 send 0·전략 로그 래치 전이 4종 = 17-11-SUMMARY D1~D4. **D-22 dev 화면 확인은 2026-09-20 에 수행했다** — 실브라우저 스크린샷 8장(`17-12-led-statusbar-{phone,wide}-{light,dark}.png` · `17-12-led-page-*.png`)에서 LED 3칩이 매수·매도·취소 순서로 보이고 라벨 대기/감시/OFF 가 동시에 노출되며, 폰 밴드(본문 **374**)에서 상태줄이 **두 줄(58px)**, 와이드 밴드(본문 **1112**)에서 **한 줄(36px)** 로 접힌다. 잘림 0 은 `trading-limit-chaser.spec.ts` 케이스 9·11·12·13 이 실브라우저로 단언하고 전부 통과했다. **Complete 로 올리지 못하는 이유(1건):** 래치 클릭의 **왕복 종단**(36/37/38 이 실제 게이트웨이에 도달하고 60 에코가 색을 바꾸는가)은 스텁이 아닌 서버 HEAD 로만 증명되는데, TRADE-04 ①과 같은 Xcode 라이선스 게이트로 mock 을 HEAD 로 세우지 못했고 실서버 배포도 미승인이다. relay↔게이트웨이 구간은 `ws-latch.test.ts` 가 `fake-gateway` 로 디코드까지 단언하므로 **미검증이 아니라 실기 미관측**이다. |
