...
## 후속 — 탭 아래 여백 재조정 (사용자 「매수주문 체크박스 위 여백이 너무 커」)

탭 여백 16px 은 색면 위 흰 여백 8px + 그림자 8px + 카드 패딩 6px 이 쌓여 체크박스까지 30px 가까이 떴다.
탭 여백은 8px 로 되돌리고, 방향 카드의 bleed 그림자 **위쪽만** `clip-path: inset(0 -8px -8px -8px)` 로 잘랐다(폰 밴드만, ≥700 은 `none`).
결과: 탭 → 흰 여백 8px → 색면 → 6px → 체크박스. 폰 매수·매도 탭 스크린샷으로 확인. vitest trading 652 · Playwright 작업대+a11y 46/0.
