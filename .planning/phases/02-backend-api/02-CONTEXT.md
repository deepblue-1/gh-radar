...
- **D-02:** 언어/런타임 통일: **TypeScript strict + Node 20**. Phase 1의 `tsconfig.base.json` extends. ES2022 타겟.
...
### 이미지 빌드 & Secret
- **D-31:** **Dockerfile 멀티스테이지** Phase 1 패턴 승계: node:20-alpine base, `pnpm deploy --filter @gh-radar/server` 로 의존성 트리밍, 최종 이미지에는 `dist/` + `node_modules` + 런타임만 포함. non-root user로 실행.
