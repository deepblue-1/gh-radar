...
- **D-08: 모든 행에 출처 칩(상따·VI·수동)을 붙이고, NXT 행에만 거래소 태그를 붙인다.** 출처 칩은 계좌 패널의 기존 출처 태그 스타일을 재사용하되, 원래 태그가 생략하는 「수동」도 표시한다. 주문번호가 없는 로컬 거부 행은 주문번호 자리에 「—」를 쓴다. 현행 「· 수동」 꼬리 표시는 없앤다. 주문자(DMA 사용자) 표시는 하지 않는다. **보충(plan-phase 2026-09-24 사용자 결정):** origin 미상 행(게이트웨이 재시작 뒤 termId=0 등)은 출처 칩을 생략한다 — 「수동」 으로 표시하면 거짓일 수 있다. 「모든 행」 은 「출처를 아는 모든 행」 으로 좁힌다.
