...
- **D-11 호가 종목정보 10칸.** `RelayQuote` 에 `kc`(krx_close_price) 추가. `kc > 0` 이면 10번째 칸 라벨 `'종가'` + 값 `kc`, 0 이면 종전 `'하락VI'`. **벽시계로 판정하지 않고 0 도 권위값이라 거르지 않는다**(같은 값이면 no-op). NXT 프레임에도 KRX 종가가 실리므로 라벨은 그대로 '종가'(C# 동일). 10칸 구성이 바뀌지 않으므로 §2.2b 밴드 경계 재측정은 불필요 — 라벨 폭만 확인.
...
- **D-21 접근성·표기.** LED 는 색만이 아니라 텍스트도 싣는다 — 각 LED 옆 짧은 라벨(예: `매수 잠복` / `매수 감시` / `매수 —`) 또는 최소 sr-only. 툴팁 문구는 C# 것을 한국어 그대로 가져온다(특히 매수 켜는 쪽: "벽이 이미 감시수량 이상이면 바로 매수 주문이 나갈 수 있다"). 클릭 가능 시 `cursor:pointer` + `aria-pressed`. 색 토큰은 `globals.css` 기존 토큰 우선(회색 `--muted-fg`), 주황·초록은 없으면 `--led-latent`/`--led-armed` 토큰을 라이트/다크 둘 다 정의.
- **D-22 배치.** 상따 화면 **상태줄**(현재 「무장 / 발주 완료 · 무장 해제」가 있는 줄)에 LED 3개를 가로로 둔다(순서 매수·매도·취소). §2.2b 4밴드 컨테이너 쿼리 안에서만 반응(뷰포트 분기 신설 금지). 폰 밴드에서도 한 줄에 들어가게 라벨은 짧게. **UI-SPEC 갱신 없이** 진행하되, LED 구현 태스크의 **첫 단계는 상태줄 standalone HTML 목업**(globals.css 토큰 인라인, 라이트/다크, 폰·데스크톱 밴드, 회색/주황/초록 3상태 + 라벨·툴팁 문구)을 열어 사용자 시각 확인을 받는 것이다(메모리 `feedback_ui_html_mockups_first`·`feedback_mockup_review_gate_before_commit`). 사용자 답 전에는 구현·커밋하지 않는다.
