...
- **D-08:** **히어로** = "상한가 종가 매수 → 다음날 시초가 매도 시" 큰 **시초가 익절 확률%** + "과거 N회 중 M회". 서브라인 = 평균 시초가 / 최악 저가 / 최근 3회 승패.
...
- **D-11:** **분포 히스토그램** = 다음날 시초가 수익률 분포. 목업 5버킷(−10\~−5 / −5\~0 / 0\~+5 / +5\~+10 / +10%+), 수익=빨강(`--up`)·손실=파랑(`--down`). 정확한 버킷 경계는 Claude 재량(목업 기준).
...
- **D-17:** 테마 선택/정렬 = **종목의 소속 시스템 테마 전부, 표본수 N(테마 풀 누적 상한가 이벤트 수) 내림차순**. 유저 테마 제외(개인화 노이즈). 카드 많으면 상위 일부 노출 + 더보기(Claude 재량). 카드 표기 = 테마명 · 익절률% · "N=… · 평균 ±x%".
...
- `scratchpad/limit-up-nextday-mockup.html` (세션 971e8b2a) — A/B/C 목업, **C안 채택**(시각 스펙 원본). 색상/레이아웃/컬럼 기준.
</canonical_refs>
