...
## 9. 2026-09-10 장중 실측 — 취소 왕복 · 오늘 주문 복원 · 전략 동기화

> §1~§8 은 **각 시점의 정직한 기록**이며 이 절에서 한 글자도 고치지 않았다. 그 뒤 2026-09-10 장중에
> 사용자와 함께 실계좌 관측이 일어났다(취소 왕복 1건 · `/me` 오늘 주문 복원 확인 · WinForms↔웹 전략 동기화
> 양방향 관찰 · relay 재배포). 이 절은 그 관측 **이후**의 재판정이며, §3·§4·§5·§8 과 이 절이 어긋나면
> **이 절이 정본**이다.
>
> 재판정 규칙은 §3 의 3가지와 §8 의 추가 규칙을 그대로 승계한다 — **"라이브가 됐으니 다 됐다"는 추정을
> 쓰지 않는다.** 항목마다 재현 가능한 실측 출력 또는 커밋 해시를 적고, 근거를 못 대면 ⚠ 로 남긴다.
> 이 절의 실측은 **읽기 전용 관측 + 사용자의 직접 관찰**이며, 이 절을 쓴 quick(`quick-260910-ogq`) 자체는
> **코드 변경 0 · 배포 0 · 실계좌 주문 0 · Supabase 쓰기 0** 이다. 주문·배포는 그 이전에 사용자 지시로
> 이미 일어난 것이고, 이 절은 그것을 옮겨 적을 뿐이다.

### 9.1 재집계 근거 (2026-09-10 실측)

| 근거 | 실측값 또는 커밋 | 무엇을 증명하는가 |
|------|------------------|-------------------|
| relay 프로덕션 와이어 로그 `sudo docker logs gh-radar-relay` (2026-09-10) — 주문 3건 | epoch `1789013833` `order.new` → 통보 `A` → `accepted` · epoch `1789013847` `order.new` → 통보 `A` → `accepted` · epoch `1789013859` `order.cancel` → 통보 `C` → `cancelled` | **취소(`C`) 왕복과 `cancelled` 전이가 실제로 일어났다.** §8 이 `org_order_no` 5/5 NULL 을 근거로 "취소 경로는 한 번도 실행되지 않았다"고 적은 바로 그 경로다. 신규 접수(`A`) 2건도 같은 세션에서 함께 관측됐다 |
| 위 3건의 **첫 통보 지연** | **32 / 22 / 21 ms** | D-22 의 5초 상한 대비 약 **1/200**. **§8 의 median 77 ms 와는 다른 계측이다** — §8 값은 `dma_orders` 의 `updated_at − created_at` 즉 **DB 측 Δ 근사**였고, 오늘 값은 **와이어 로그에 찍힌 첫 통보 지연**이다. 두 수를 같은 축에 놓고 "빨라졌다"고 읽지 말 것 |
| 같은 로그의 계좌번호 표기 | **말미가 가려진 마스킹 형태로만** 등장 (원본 자릿수는 이 문서에 옮기지 않는다) | R2-CR-03(계좌번호 마스킹) 수정이 **프로덕션에서 동작**한다. 문서 머리말의 비밀 기록 금지 조항을 이 절도 그대로 지킨다 |
| 프로덕션 `/me` 「오늘 주문」 카드 — **사용자 확인** | 오늘 주문 **3건 표시** (접수 2 · 취소 1) | `GET /api/orders` 의 **복원 응답이 실제로 화면에 도달**했다. §8 이 몰랐던 사실 한 줄: 이 잔여는 **검증 공백이 아니라 기능 공백**이었다 — 서버 라우트는 살아 있었으나 `webapp/src` 에 **호출자가 0건**이었다(16-16 이 주문 **접수**만 wss 로 옮기고 REST **복원** 경로는 옮기지 않았다). quick-260910-jce 가 `/me` 에 카드를 배선해 닫았고, quick-260910-kql 이 종목명을 병기했다 |
| relay 로그 `[HUB] 상따 에코 수신` **6건** (`crud` C×4 · D×2, 캐시 갱신 동반) + **사용자의 WinForms↔웹 양방향 직접 관찰** | 웹 `/trading/limit-chaser` 조작 → WinForms 반영 · WinForms 조작 → 웹 반영 (사용자 관찰) | **같은 DMA 세션에서 전략이 공유된다**(TRADE-03 잔여 1건). **한계를 그대로 적는다:** relay 는 **성공한 인바운드 전략 요청을 로그로 남기지 않으므로**(거부만 기록) **각 에코의 발신 방향을 로그만으로는 가릴 수 없다.** 방향 판정의 근거는 **사용자의 직접 관찰**이며, 이 항목은 `16-VERIFICATION.md` §Human Verification Required #1 이 애초에 **human-only** 로 지정한 것이라 **그것이 의도된 증거 형태**다. 로그는 결론이 아니라 **메커니즘**(같은 세션에서 전략 에코를 받는다)을 뒷받침한다 |
| relay 재배포 `11072e4` (장중, **사용자 승인** 하에) + 배포 후 `/healthz` | `{"status":"ok","vpn":true,"dma":true,"version":"11072e4","sessionCount":2,"everReadyCount":2,"stalledCount":0}` · `DMA_HOST` = `10.41.1.120:9100` **무주입 보존** 확인 | 배포 이미지 태그 = `version` 일치 → 배포가 실제로 반영됐다. **Phase 16 갭 5 수정(`DMA_HOST` 를 재배포가 덮어쓰지 않는다)이 두 번째 배포에서도 동작**한다. `everReadyCount` 2 · `stalledCount` 0 은 두 세션이 모두 Ready 에 도달했고 멈춘 세션이 없다는 뜻 |

> **강한 주장으로 부풀리지 않는다.** 위 표의 주문 3건은 **사용자 지시로 사용자가 낸 실주문**이고, 이 절을 쓴
> quick 은 주문을 내지 않았다. `/me` 표시와 WinForms↔웹 동기화의 관측 주체도 **사용자**다 — Claude 가 화면을
> 본 것은 아니다. §8 이 §4-C·§4-D 에서 지킨 표기 규율을 그대로 따른다.

### 9.2 SC-4 · SC-6 재판정

**SC-1·SC-2·SC-3·SC-5·SC-7·SC-8 은 재판정 대상이 아니다.** §8(2026-09-08)에서 이미 ✅ 였고, 오늘의 관측은
그 근거를 바꾸지 않는다. 아래 두 행만 재판정한다.

| SC | 2026-09-08 판정 | 2026-09-10 재판정 | 근거 | 남은 미증명 |
|----|----------------|-------------------|------|-------------|
| **SC-4** | ⚠ 부분 충족 | **⚠ 부분 충족 유지** | 오늘의 관측은 SC-4 의 잔여를 건드리지 않았다. 배포 후 `sessionCount` 2 는 **소켓이 열려 있다**는 뜻이지 **유예 동작의 관측이 아니다** — 이 값을 5분 유예의 증거로 쓰지 않는다 | **마지막 wss 종료 5분 뒤 DMA 세션이 닫히는 것의 실시간 관측. 오늘도 하지 않았다.** 해소 조건은 §8.5 와 동일: 세션 보유자가 마지막 wss 를 닫은 시각을 특정하고 5분 경과 후 `/healthz` 의 `sessionCount` 가 `0` 인지 확인 |
| **SC-6** | ⚠ 부분 충족 (6항 중 4항 해소) | **✅ 충족** | §8 이 남긴 잔여 **2건이 둘 다** 닫혔다 — ① **취소(`C`) 왕복 + `cancelled` 전이**: `order.cancel` → 통보 `C` → `cancelled` 를 relay 와이어 로그에서 관측(9.1) · ② **`GET /api/orders` 오늘 주문 목록 복원 응답**: 프로덕션 `/me` 에서 오늘 주문 3건(접수 2 · 취소 1) 표시를 사용자가 확인(9.1). §8 이 이미 증명한 4항(`dma_orders` 실왕복 · 실브로커 rc · 전이 Δ ≤ 5초 · `isin` 매핑 · 매수/매도 양방향 · 409)은 그대로 유지 | 없음 |

**집계 (2026-09-10):** ✅ **7** (SC-1·SC-2·SC-3·SC-5·SC-6·SC-7·SC-8) · ⚠ **1** (SC-4) · ❌ **0**
*(2026-09-08: ✅ 6 · ⚠ 2 · ❌ 0 — 2026-09-06: ✅ 3 · ⚠ 5 · ❌ 0)*

### 9.3 요구사항 판정으로의 전달 (REQUIREMENTS.md 는 이 결론을 옮긴다)

- **RELAY-02 → Complete.** 열거 조항이 **전부** 근거를 얻었다. 신규 매수/매도 릴레이 · `OrderResp` ≤5초 ·
  체결(`E`) 푸시 · `dma_orders` 기록 · ISIN 매핑 · `409 SESSION_NOT_READY` 는 **§8 에서**, 잔여였던
  **취소 릴레이 + 취소확인(`C`) 푸시**와 **오늘 주문 목록 복원**은 **오늘 §9.1 에서** 채워졌다.
  **정의문 대조 한 줄:** RELAY-02 정의문의 `POST /api/orders` 는 Phase 16(16-16)이 **wss 단일 경로로
  의도적으로 대체**했고 REST 는 `GET`(복원)만 남았다 — **결손이 아니라 설계 갱신**이다. 판정은 정의문의
  경로 문자열이 아니라 **조항의 기능(주문 릴레이 · 통보 반영 · 오늘 주문 복원)이 성립하는가**로 한다.
  다음 사람이 정의문과 코드의 불일치를 미완으로 오독하지 않도록 여기에 남긴다.
- **TRADE-03 → Complete.** 잔여였던 **「WinForms ↔ 웹 한 세션 동기화」가 사용자의 직접 관찰로 성립**했다
  (양방향). 증거 형태가 **human-only** 라는 점과 **로그 코러버레이션의 한계(에코의 발신 방향을 로그만으로는
  판별할 수 없다)** 를 함께 기록한다 — 승격의 근거와 그 근거의 약한 지점을 같이 남기지 않으면, 나중에
  "무엇을 보고 초록으로 칠했는가"를 복원할 수 없다. 이 요구사항의 **정본 근거 문서는 Phase 16 쪽**
  (`16-VERIFICATION.md` §Human Verification Required #1 · `16-VALIDATION.md` §Gap Closure)이며,
  **이 절은 relay 측 메커니즘 근거**(같은 DMA 세션에서 전략 에코 6건 수신)를 제공하는 보조 위치다.

### 9.4 이관 (새로 열리는 항목)

1. **거래원 푸시(74/75) 드롭 로그 분리의 프로덕션 실측 미완.**
   *무엇이 미완인가:* quick-260910-jce 가 범위 밖 `msg_type` 중 **응답 대역 5종만** debug 로 강등하고
   요청 대역(20·26·27·30·31)은 WARNING 으로 남기도록 고쳐 단위 테스트로 잠갔으나, **프로덕션 확인은 불발**이다.
   *왜:* 재배포 후 구독이 **17:33 KST(장 마감 후)** 에 일어나 거래원 푸시가 **아예 오지 않았다**. `LOG_LEVEL=info`
   라 debug 는 애초에 보이지 않는다. 따라서 **관측된 WARNING 0건은 「수정됐다」가 아니라 「75가 안 왔다」는 뜻**이다.
   *닫으려면:* **다음 장중(2026-09-11 금 09:00~15:30)에 호가주문 탭을 연 상태로**
   `sudo docker logs gh-radar-relay | grep -c "unknown-msg-type"` 가 0 인지와 **구독이 실제로 발생했는지**를
   **함께** 대조한다. 구독 발생을 확인하지 않은 0 은 아무것도 증명하지 않는다.

   > **[2026-09-11 종결]** 위 절차대로 **장중에 대조해 닫혔다.**
   > *실측:* 새 컨테이너 `11072e4` · 구독 활성(`KR7117670000` KRX, **10:15:46 KST 시작**) ·
   > **장중 4.5분**(10:15:46~10:20:10) 동안 `unknown-msg-type` **0건** · `프레임 드롭` WARNING **0건**.
   > *대조군:* 옛 컨테이너 `a1f4ed6` 는 **같은 조건**(구독 활성 · 장중)에서 40분에 **13건**(25~55초 주기)이었다.
   > 4.5분이면 5~10건이 떴을 구간이고, 조건이 같고 바뀐 것은 코드뿐이므로 전후 대조가 성립한다.
   > 「구독 발생을 확인하지 않은 0」이라는 이 항목 자신의 함정도 피했다 — 구독 시각을 먼저 확인하고 셌다.
   >
   > **다만 증거를 부풀리지 않는다.** `LOG_LEVEL=info` 라 debug 로 내린 `out-of-scope` 분류는
   > **직접 관측하지 못했다.** 증명된 명제는 **「WARNING 이 사라졌다」**이지
   > **「75가 들어와 out-of-scope 로 분류됐다」가 아니다.** 후자는 단위 테스트에만 잠겨 있다.
   > 후자를 보려면 `LOG_LEVEL=debug` 로 컨테이너를 재시작해야 하는데, 장중 세션 단절 비용 대비
   > 값어치가 없다고 판단해 하지 않았다.
2. **relay 관측성 공백 — 성공한 인바운드 전략 요청이 로그에 없다.**
   *무엇이 미완인가:* relay 는 전략 요청을 **거부할 때만** 로그를 남긴다. 성공 경로가 무로그라 전략 에코의
   **발신 방향을 사후 판별할 수 없다.**
   *왜 중요한가:* 오늘 TRADE-03 의 방향 판정을 **사용자 관찰에 의존하게 만든 원인**이 이것이다. 같은 관측을
   나중에 재현하려는 사람은 로그만으로는 같은 결론에 도달할 수 없다.
   *닫으려면:* 성공한 인바운드에도 최소 1줄(요청 종류 + `crud` + 상관 ID; **계좌·가격 값은 제외**)을 남기도록
   relay 를 고친다. **이 quick 의 범위 밖이며 코드 변경이다.**
3. **`npm run lint` 기존 경고 3건 (선재 · 범위 밖).**
   `webapp/src/components/theme/theme-detail-client.tsx:9` · `webapp/src/components/trading/__tests__/strategy-status-card.test.tsx:329`
   · `webapp/src/lib/use-relay-socket.ts:808`. quick-260910-kql SUMMARY §범위 밖 발견에서 승계한다.
   **오늘 손대지 않았다.** *닫으려면:* 별도 정리 작업으로 다루되, 이 문서의 SC 판정과는 무관하다.

### 9.5 재집계 후에도 남는 것

§8.5 의 목록을 오늘 기준으로 갱신해 다시 적는다(**§8.5 자체는 고치지 않는다** — 그것은 2026-09-08 시점의 기록이다).

- **SC-4 — 마지막 wss 종료 5분 뒤 세션 종료 실관측. 유지.** *닫으려면:* 세션 보유자가 브라우저를 닫은 시각을
  특정하고, 5분 경과 후 `curl https://dma.jx1.io/healthz` 의 `sessionCount` 가 `0` 인지 확인한다. 읽기 전용이며
  실계좌 위험 없음. **오늘도 하지 않았다.**
- **§5-8 알림 임계값 0.9 (원 권고 0.5~0.7). 유지.** *닫으려면:* 라이브 운영 중 단일 리전 순간 실패로 인한
  오경보를 실제로 관측한 뒤 그 관측을 근거로 조정한다. 관측 없이 미리 낮추지 않는다.
- **9.4 의 이관 3건** — 거래원 푸시(74/75) 드롭 로그 프로덕션 실측 · relay 성공 인바운드 무로그 · lint 선재 3건.
- **(판정 아님, 기록만) 2026-09-08 19:17 KST 체결 통보 2건의 시각 성격. 승계.** 통보가 왔다는 사실만 남기고
  해석하지 않는다.
- **빠진 것:** §8.5 가 들고 있던 **SC-6 두 항목**(취소 `C` 왕복 + `cancelled` 전이 · `GET /api/orders` 복원 응답)은
  **오늘 닫혔으므로 이 목록에서 빠진다**. §8.5 의 원문은 그대로 두되, 현재 유효한 잔여 목록은 이 §9.5 다.
