...
## 부록 2 — 3라운드 갭 클로징 결과 (2026-09-09, 16-46 실행자 추가)

> **이 부록은 덧붙인 것이다.** 위 본문과 부록 1(orchestrator 작성)은 **검증 시점의 기록**이며
> 한 줄도 고치지 않았다. frontmatter 의 `gaps`·`status` 도 건드리지 않았다 — 그것은 검증자의
> 판정이지 실행자가 덮어쓸 것이 아니다. 아래는 「그 판정이 어느 plan 으로 닫혔는가」만 가리킨다.

**처리 결과의 정본은 `16-VALIDATION.md` §Gap Closure 3라운드 (16-36 ~ 16-46) 의 17행 표다.**
아래는 이 문서가 제기한 항목과 담당 plan 의 대응만 적는다.

### 본문 §Gaps Summary 의 갭 3건 (= R2-CR-01·02·03)

| 이 문서의 갭 | ID | 담당 plan | 결과 |
|--------------|----|-----------|------|
| **갭 1** — `crud:"D"` 스푸핑이 시장·무장 가드를 동시 우회 | R2-CR-01 | **16-36** (`eeb4539`·`51408ee`) | 닫힘. 이 문서가 제시한 수정 방향(「`#isTeardown` 을 게이트 4종으로 판정」)을 그대로 채택했다. `fanout.ts:841-845` 반환식이 `crud` 를 참조하지 않음을 16-46 이 코드에서 재확인 |
| **갭 2** — 자격증명 거부 세션 1건이 relay 전체를 영구 503 으로 | R2-CR-02 | **16-37** (`f2bdb48`·`d6f1efb`) | 닫힘. 제시된 수정(「`stalledCount` 집계에서 `NO_RETRY_STATES` 제외」) 그대로. `session-manager.ts:305` 확인 |
| **갭 3** — CHECK/FK 위반 로그가 계좌번호를 유출 | R2-CR-03 | **16-38** (`dc19f09`·`84c23d7`·`110bbb7`) | 닫힘. 제시된 수정(「안전 필드만 뽑는 공용 헬퍼 하나로 세 곳 교체」)보다 **넓게** 닫혔다 — 전수 조사로 **13곳**을 교체했고, 16-46 재실측에서 relay 전체 **15곳**(이후 plan 이 2곳 추가) |

> **이 문서의 「진행 판단」이 단 조건이 발동했고, 충족됐다.** 본문은 「실서버 결선 이전에
> 반드시 닫아야 한다 — 특히 갭 1(무장 가드 우회)과 갭 3(계좌번호 유출)은 결선하는 순간
> 실계좌·실개인정보 위에서 도는 경로다」라고 적었다. 부록 1 이 기록했듯 결선은 검증 **직후**에
> 일어났고, 3건은 그 상태에서 닫힌 뒤 `a1f4ed6` 로 프로덕션에 배포됐다.

### 부록 1 의 새 갭 2건

| 갭 | 담당 plan | 결과 |
|----|-----------|------|
| **갭 4** — 사이드바 상따 전략이 종목명 대신 ISIN 으로 표시 | **16-41**(relay) + **16-42**(webapp) | 닫힘. 부록 1 이 제시한 방향(「원천을 늘리지 않고 relay 의 `SymbolMap` 으로 60/61 에 `name`·`code` 를 얹는다」)을 그대로 채택했다. 실제 구현은 60(에코)·**64(스냅샷)** 둘 다, **캐시 삽입 이전**에 보강한다 — 팬아웃만 보강하면 「지금 화면」은 이름이 있고 「새로 연 탭」(`lc.snap` 재접속 복원)은 ISIN 이 되기 때문이다. **T-16-02 는 지켜졌다**(웹앱에 새 조회 경로 0건). 16-46 이 프로덕션 청크 `2422-ff4cf64d654c9829.js` 안의 `useIsinLabels` 를 **내용으로** 확인해 라이브임을 증명했다 |
| **갭 5** — `deploy-relay.sh` 가 현재 `DMA_HOST` 를 보존하지 않음 | **16-45** (`9329e1b`) | 닫힘 + **프로덕션 실증됨.** 부록 1 이 제시한 방향(「배포 전 실행 중 컨테이너의 `DMA_HOST` 를 읽어 기본값으로 삼고, 명시 주입이 우선, 값이 바뀌면 변경 전/후를 나란히 찍는다」) 그대로다. 16-45 자신은 `gcloud` 를 한 번도 부르지 않았고(원문 추출 + 로컬 `source` 검증), **16-46 의 무주입 배포가 유일한 실 VM 실증**이다 — `DMA_HOST=10.41.1.120` 이 배포를 넘어 보존됐고 강등 경고는 출력되지 않았다 |

### 본문 §Anti-Patterns Found 의 Warning 5건

| File · Line | ID | 담당 plan |
|---|---|---|
| `order-handler.ts:1043-1046` (`noticeType` 블랙리스트) | R2-IN-03 | 16-43 (`8bcf01c`) — 화이트리스트로 전환 |
| `order-handler.ts:1015-1026` (`orgOrderNo` 완전일치) | R2-WR-03 | 16-43 (`8bcf01c`·`31c0fd4`) — 정규화 + 축 전멸 로그 |
| `limit-chaser-form.tsx:495-500` (「수정」 철거 면제 없음) | R2-WR-02 | 16-42 (`e803355`) |
| `fanout.ts:1044-1059` (state 리스너 누적) | R2-WR-05 | 16-44 (`5ee91f6`·`dac2856`) — **실제 누수 지점은 `#onClose` 였다** |
| `orders.ts:284-291, 618` (23505 갱신 예외) | R2-WR-01 | 16-39 (`f0dd2a0`) + 16-40 (`e32ca65`) |

나머지 R2-WR-04·06·07 · R2-IN-01·02·04·05 도 전부 닫혔다 — 담당 plan 과 근거는 위 정본 표 참조.

### 본문 §반증 패스가 지목한 3건 — 전부 조치됨

1. **「부분적으로만 충족된 요구사항: TRADE-03」** → 여전히 Pending 이나 **사유가 갱신됐다.**
   「프로덕션에서 한 프레임도 나른 적이 없다」는 더 이상 참이 아니다(`everReadyCount: 1` 실측).
   잔여는 **「WinForms ↔ 웹 한 세션 동기화 미실측」 1건**이다. 사용자 결정(2026-09-09).
2. **「통과하지만 위험을 검사하지 않는 테스트: `fanout.test.ts` ⑰-e/⑰-e2」** → 16-36 이
   **정정했다.** 두 케이스는 헬퍼 기본값 `buyEnabled: true` 때문에 「진짜 철거」가 아니라 이
   갭이 지목한 스푸핑 조합을 태우면서 초록이었다. **수정을 약화시켜 통과시키지 않고 테스트를
   진실로 바꿨다.** 새 회귀 잠금 3케이스(⑰-e3·e4·e5)를 더했다.
3. **「테스트 커버리지가 없는 에러 경로」** → `session_rejected` 의 `stalledCount` 영향은
   16-37 이 ⑩-b·⑩-c 로, `order-store` 의 로깅 내용 단언은 16-38 이 ⓼-b~⓼-e 로 잠갔다.

### 본문 §Human Verification Required — 상태

6건 중 상태가 바뀐 것은 **없다.** #1(WinForms ↔ 웹 세션 공유)에 이 문서가 단 경고
(「R2-CR-01·R2-CR-03 이 닫히기 전에 실행하면 실계좌 위험」)의 **조건은 해소됐다** — 두 갭이
닫혀 `a1f4ed6` 로 배포됐다. 그러나 검증 자체는 여전히 미실행이고 D-27 상 사용자 명시 지시가
필요하다. #2(smoke INV-9)도 미실행이다 — **「토큰이 없어 프로브 본체가 한 줄도 실행되지
않았다」**이지 「돌렸는데 SKIP」이 아니다. 상세는 `deferred-items.md` §16-46.

_부록 2 작성: 2026-09-09, 16-46 실행자 (갭 클로징 3라운드 종결 plan)_
