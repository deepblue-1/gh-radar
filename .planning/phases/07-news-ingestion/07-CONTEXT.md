...
## Decision Revisions (2026-04-17, plan review 후 · D2/D3 수집 정책 수정)

### R6. D2 revised — 배치 주기 장중/장외 분리
- 원본 D2: `*/15 * * * *` (KST) 24시간 단일 주기
- **갱신:** Cloud Scheduler **2개** 로 분리 운영. 시간 구간 중복 실행 시 ON CONFLICT DO NOTHING 이 중복을 흡수하므로 리스크 없음

  | Scheduler | cron (KST) | 시간대 | 목적 |
  |-----------|------------|--------|------|
  | `gh-radar-news-sync-intraday` | `*/15 9-15 * * 1-5` | 장중 (평일 09:00~15:59) | 급등/급락 반영에 15분 샘플링 |
  | `gh-radar-news-sync-offhours` | `0 */2 * * *` | 전 시간 (장외 평일 + 주말 전체) | 공시/실적/외신 2시간 샘플링 |

- 두 scheduler 는 동일한 Cloud Run Job(`gh-radar-news-sync`) 을 트리거. OAuth invoker + `gh-radar-scheduler-sa` 유지 (Pitfall 2 — OIDC 금지)
- **주의:** CONTEXT D2 원문에 "Cloud Scheduler OIDC invoker" 문구가 있으나 이는 오타 성격 — STATE.md Phase 05.1 Pitfall 2 에 따라 **OAuth 채택이 정답**. Plan 측은 이미 OAuth 사용.

### R7. D3 revised — display=100 + 페이지네이션 + 증분 종료조건
- 원본 D3: `display=20`, `start=1`, `sort=date` (한 번만 호출)
- **갱신:**
  - `display=100` (최대치), `sort=date`, `start=1 → 101 → 201 → ...`
  - **페이지네이션 루프** — 종목당 다음 중 하나를 만나면 종료:
    1. **증분 종료조건:** 이번 페이지의 모든 기사가 `news_articles.MAX(published_at where stock_code=X)` 이하 → 종료 (첫 수집 시엔 생략)
    2. **첫 수집 컷오프:** `MAX(published_at)` 이 없는 경우(첫 수집), 최근 **7일 이내** 기사만 저장하고 7일 초과 만나면 종료
    3. **API 하드 상한:** `start + display > 1000` 또는 API 가 400 반환 시 종료 (Naver 도달 한계)
    4. **응답 빈 배열:** 더 이상 기사 없음
- 자연 방어선: API `start ≤ 1000` 하드 상한. 별도 종목당 cap 미도입(사용자 선택 A).
- **예상 호출량 재계산**
  - 장중 (평일): 26 tick × 200종목 × 평균 1.3 pages ≈ **6,760 calls**
  - 장외 평일: 8 tick × 200 × 1.3 ≈ **2,080 calls**
  - 장외 주말: 12 tick × 200 × 1.3 ≈ **3,120 calls**
  - 주간 합계 ≈ 50,440, **일 평균 ≈ 7,200 calls** (25K 한도의 **29%**, 기존 77% → 완화)
- `NEWS_SYNC_DAILY_BUDGET=24500` 안전 마진 유지. budget 선제 체크는 tick 시작 시 1회 + per-page atomic increment 로 강화.

---

*Phase: 07-news-ingestion*
*Context gathered: 2026-04-17*
*Revised: 2026-04-17 (R1–R5, R6–R7)*
