...
### 탭 목적지와 앱 안 웹 셸
- **D-06:** 탭 5개 = **홈 `/` · 검색 `/search` · 트레이딩 `/trading` · AI `/chat` · 마이 `/me`.** 탭 탭(tap) = WebView 에 해당 경로 로드(같은 탭 재탭 = 그 경로로 다시 이동, 스크롤 최상단). → D-32 탭 루트 스크롤 복원 · 스켈레톤 없는 재방문(재탭 = 맨 위 유지).
- **D-06a (2026-09-25 리서치 반영):** 탭 탭(tap)은 **웹 훅 `window.__ghTrade.navigate(path)`(Next.js `router.push` + 스크롤 최상단)를 먼저 부르고, 훅이 없을 때만 WebView 전체 로드**로 폴백한다 — 매 탭마다 전체 로드하면 relay 소켓이 끊기고 다시 붙는다. D-04 refresh 훅과 같은 `window.__ghTrade` 네임스페이스.
...
- **D-08a (2026-09-25 목업 검토 확정 · 스케치 005):** `/me` 계정 카드 = **A 「한 줄 + 아이콘 버튼」**. `--card` radius 16 · padding 14/16 · 높이 72 · 아바타 44(`avatar_url` → 이니셜) · 이름 16/700 · 이메일 12.5 muted · 우측 40×40 radius 12 `--raised` 아이콘 버튼 2개: 테마(Sun/Moon · `aria-label="테마 전환"`) · 로그아웃(LogOut · `--up` · `aria-label="로그아웃"`). 카드 아래 16 여백 후 기존 순서(상태줄 → 전략 현황 → 계좌 카드) 불변. 웹·앱 공통. → D-08b 가 테마 버튼 아이콘 규칙·접근 이름을 대체.

### 탭바 표시 규칙
- **D-12:** **숨김 = ① URL 이 `/login`·`/auth/*` ② 오프라인 폴백 화면 ③ 웹이 「오버레이 열림」 신호를 보낸 동안.** 오버레이 = 트레이딩 바텀시트·키패드 시트(Phase 20 `NumberPadSheet` 류) · shadcn `Sheet`(사이드바 드로어 · 모바일 하단 패널) · `Dialog`(주문 확인). 웹은 열림/닫힘을 네이티브에 메시지(`overlay: true|false`)로 보내고 네이티브는 150ms 지연 후 0.2s 페이드(weekly-wine `filterState` 방식). 채널: iOS `webkit.messageHandlers.ghTrade.postMessage` · Android `window.GhTradeBridge` (`addJavascriptInterface`). 웹은 두 채널을 감싼 `postNative(type, payload)` 하나만 쓴다. → D-12a 키보드 숨김 즉시 · D-12b 문서 로드 중 대기. → 2026-09-26 D-12a'' 가 키패드 시트(NumberPadSheet) 열림 숨김만 즉시로 대체.
...
### 새로고침 세부
- **D-16:** **`/trading` · `/me` 의 refresh 훅 = relay 재탐침 + 스냅샷 재요청.** `use-relay-socket` 의 resume probe(`RELAY_RESUME_PROBE_MS` 경로)를 강제 실행 — 소켓이 죽었으면 재연결, 살아 있으면 전략·잔고·미체결 스냅샷 재요청. 편집 중 상태(수동주문 입력)는 유지. `RelayProvider` 가 `probeNow()` 류 메서드를 노출한다. **(2026-09-25 리서치 반영)** relay 인바운드 프로토콜에 「스냅샷 재요청」 프레임이 없으므로(`relay/src/ws/protocol.ts`) **relay 는 무변경** — `probeNow()` = 기존 resume 경로(`onResume(true)`) 강제 실행만. 소켓이 살아 있으면 서버 푸시가 곧 최신이고, 죽어 있으면 재연결이 초기 스냅샷을 다시 받는다.
...
- **D-19:** **오프라인 = 앱 내장 폴백 화면 + 자동 복구.** `mobile/www/index.html`: GH Trade 워드마크 · 「인터넷 연결을 확인해주세요 · 복구되면 자동으로 이동합니다」 · 「다시 시도」 버튼 · `online` 이벤트 + 5초 폴링으로 `server.url` 복귀(weekly-wine `www/index.html` 이식). 탭바 숨김(D-12 ②). 첫 로드 실패(`didFailProvisionalNavigation` / `onReceivedError` 메인프레임)에 이 페이지를 띄운다. **(2026-09-25 리서치 반영)** Capacitor `server.errorPath` 는 쓰지 않는다 — iOS 는 취소(-999)에도, Android 는 Next.js 404 같은 HTTP 4xx/5xx 에도 폴백을 띄워 오동작한다. 네트워크 오류 코드에만 반응하는 자체 처리(iOS 내비게이션 델리게이트 전달 · Android `BridgeWebViewClient` 서브클래스)로 간다.
...
- **D-25:** **safe-area 는 웹이 책임진다.** `viewport-fit=cover` 추가(`viewport` export), 헤더 `padding-top: env(safe-area-inset-top)`, 하단 고정 요소에 `env(safe-area-inset-bottom)` — `dirty-action-bar` · `shared-panels` 모바일 하단 패널(`shared-panels.tsx` 하단 fixed) · 종목상세 주문 CTA 바(`stock-detail-tabs.tsx` 하단 fixed) · `chat-fab` · `alert-toasts` · `ui/sheet` bottom. **(2026-09-25 리서치 반영)** 앱에서는 이 하단 고정 바들이 네이티브 탭바에 가리므로 **공용 CSS 변수 하나(`--native-tabbar-offset`, `html.native-app` 에서 70+20+여백)** 로 전부 탭바 위로 올린다. Android 는 Capacitor 8.5 내장 `SystemBars` 가 `--safe-area-inset-*` CSS 변수를 주입하므로 웹 CSS 는 `var(--safe-area-inset-bottom, env(safe-area-inset-bottom))` 형태로 쓴다. **앱에서는 본문 하단에 탭바 높이+여백(≈ 70 + 14 + inset)만큼 `padding-bottom`** 을 `html.native-app` 에서 준다(weekly-wine 은 카페24 CSS `padding-bottom:100px` 가 담당). WebView 는 `contentInsetAdjustmentBehavior = .never` + 상태바 아래부터 frame(weekly-wine 방식) 대신 **풀블리드 + 웹 safe-area** 로 간다 — 헤더 블러가 상태바 뒤까지 이어지는 토스식 표현. → D-25a 앱 /trading 하단 공용 패널 숨김 · 종목상세 CTA 는 D-30 「트레이딩」.
...
- **D-27a (2026-09-25 목업 검토 확정 · 스케치 004):** **탭바 = A 알약**, **아이콘 = A 다크 레이더**. 네이티브 상수 정본: 좌우 여백 16 · 높이 70 · radius 32 · 배경 `--card` 82% + 블러 18 · 그림자 `0 2px 16px rgba(0,0,0,.18)` + 1px `--line` inset · 바닥 = **화면 끝에서 20pt/dp**(iOS: `safeAreaLayoutGuide.bottomAnchor` 에 `constant: +14` = 인셋 34 기기에서 20 · 인셋 0 기기(SE·3버튼 Android)는 14 하한 · Android 제스처 내비 = `bottomInset − 14`, 하한 14) · 탭바 상단부터 120 하단 페이드(`--bg` 92%) · 활성 = `--primary` 14% 원 46 + filled 아이콘 + primary 라벨 10px · 비활성 `--muted-fg` · iPad 폭 560 가운데 · 웹 본문 하단 여백 108(= 70 + 20 + 18, 폰·iPad 공통). 색은 `toss-dark.css`/`toss-light.css` 토큰 값을 hex 로 옮긴다(다크 card `#202027` · bg `#17171c` · primary `#3485fa` · muted `#9e9ea4` · line rgba(255,255,255,.07) / 라이트 card `#ffffff` · bg `#ffffff` · primary `#3182f6` · muted `#6b7684` · line `#e5e8eb`). 아이콘 원본 = `.planning/sketches/004-native-tab-bar/index.html` 의 `#app-a` symbol(1024) — `mobile/resources/icon.svg` 로 추출해 PNG 렌더. 스플래시 = 바탕(라이트 `#ffffff`/다크 `#17171c`) + 아이콘 96 + 「GH Trade」 워드마크. → 2026-09-26 D-27b 가 탭바 상수(높이 · radius · 배경 · 테두리 · 그림자 · 활성 표시 · 라벨 · 페이드 · 본문 여백)를 대체.

### 갭 클로징 결정 (UAT 2차 · 2026-09-26 사용자 확정)
- **D-27b (G-21-1 · 스케치 007 C — D-27a 의 탭바 상수를 대체):** 라벨 없음(접근 이름은 iOS `accessibilityLabel` · Android `contentDescription` 유지) · 아이콘 26 세로 가운데 · 활성 = 아이콘 뒤 캡슐 56×36 radius 18 `--primary` 16% + filled 아이콘 + primary · 비활성 `--muted-fg` · 테두리 없음 · 유리 = glass-hi(다크 `#2c2c35` 62% · 라이트 `#ffffff` 72%) + 블러 28 + 채도 1.8(iOS = ultra-thin 재질 + glass 틴트 · Android = 실블러 없음 → glass RGB 94% 불투명 근사, A10 선례) · 그림자 `0 4px 24px rgba(0,0,0,.14)` + `0 1px 2px rgba(0,0,0,.08)`(iOS 두 겹 · Android elevation 12dp 근사) · 높이 60 · radius 30 · 좌우 16 · iPad 폭 560 가운데 · 바닥 max(inset−14, 14) 불변 · 하단 페이드 86(= 60 + 26, `--bg` 80% @75%) · 웹 `--native-tabbar-offset` = gap + 60 + 8 · 본문 하단 여백 98(= 60 + 20 + 18). D-27a 의 색 토큰 · 좌우 · 바닥 · iPad · 아이콘/스플래시는 유지. 수치 정본 = `.planning/sketches/007-tab-bar-no-label/` Winner. — **Reversibility:** reversible. 근거 = 21-UAT.md G-21-1.
- **D-23a (G-21-N1):** 저장된 테마가 없을 때 기본 = **다크**, 웹·앱 공통 — 웹 next-themes 기본 dark(`enableSystem=false` 유지) · iOS/Android `ThemeStore` 미저장 dark · 오프라인 폴백(`mobile/www/index.html`)은 `?theme=light` 가 아니면 다크 · 네이티브 사전 로드 자리값(iOS VC `currentTheme` · 탭바 초기 팔레트)도 dark. 저장값이 있으면 그대로(기본값은 저장하지 않는다). 스플래시(OS 런치 화면)는 D-22 대로 OS 외형을 따른다. 근거 = 21-UAT.md G-21-N1.
- **D-08b (G-21-N2 — D-08a 의 테마 버튼 접근 이름을 대체):** 테마 버튼 아이콘 = **누르면 바뀔 테마(목적지)** — 다크일 때 Sun · 라이트일 때 Moon. 사이드바 `ThemeToggle` 과 `/me` 계정 카드 공통 · 접근 이름 = 행동 문구 「다크 모드로 전환」/「라이트 모드로 전환」 · 정의는 `webapp/src/components/layout/theme-toggle.tsx` 의 `ThemeSwitchIcon` · `THEME_SWITCH_LABEL` 한 곳(새 표면은 가져다 쓴다 — 문구·아이콘을 직접 적지 않는다). 근거 = 21-UAT.md G-21-N2.
- **D-28 (G-21-N3):** 앱에서 서버 호스트(`server.url` 호스트, 운영 trade.jx1.io) 밖 http(s) 최상위 이동·새 창 요청 = **인앱 브라우저**(iOS `SFSafariViewController` · Android Custom Tabs). mailto · tel · 앱 딥링크 등 비 http(s) 는 기존 Capacitor 기본(시스템). 같은 호스트는 WebView 유지(`target=_blank` 포함). 방식 = 네이티브 셸 가로채기(iOS `NavigationDelegateProxy` · Android `GhTradeWebViewClient`) · 웹 코드 무수정 · npm 패키지 추가 없음 · `allowNavigation` 은 계속 비움 · 시스템으로 넘길 예외 호스트 없음(21-15 네이티브 로그인). Android Custom Tabs 구현 경로 = 21-17 Task 1 결정 **선택 a** — `androidx.browser` 1.9.0 을 앱 모듈에 선언(capgo 가 이미 APK 에 싣는 좌표와 같아 런타임 의존 집합 무변경, 21-17-SUMMARY). — **Reversibility:** reversible. 근거 = 21-UAT.md G-21-N3.

### 갭 클로징 결정 (UAT 3차 · 2026-09-26 사용자 확정)
- **D-12a (G-21-R3-1 — D-12 의 키보드 숨김 타이밍 대체):** 키보드 사유 숨김 = 150ms 대기 없이 **즉시** · 키보드보다 짧은 페이드(iOS = `userInfo` 애니메이션 시간의 절반 · 같은 곡선, Android = IME 애니메이션 `onPrepare` 에서 즉시) · 아래로 내려가는 이동 없음 · 재표시 90ms 디바운스 · iOS 는 키보드 끝 프레임이 화면 하단을 가릴 때만 키보드로 본다. 오버레이 · 경로 · 오프라인 숨김은 D-12 그대로. — **Reversibility:** reversible. 근거 = 21-UAT.md G-21-R3-1. → 2026-09-26 D-12a' 가 키보드 숨김 페이드 길이를 대체(즉시 · 애니메이션 없음).
- **D-12a' (2026-09-26 · quick-260926-v5n · 사용자 실기기 보고 — D-12a 의 키보드 숨김 페이드 길이만 대체):** 키보드 사유 숨김 = **애니메이션 없이 즉시** — 두 플랫폼 같음. 옛 값: iOS = 키보드 애니메이션 시간의 절반(0.08~0.2초) · 키보드 곡선(`userInfo`) / Android = IME 애니메이션 절반(80~200ms) · IME 보간기. 새 값: 0 — iOS 는 `UIView.performWithoutAnimation` 으로 키보드 애니메이션 트랜잭션 상속까지 끊는다 · Android 는 애니메이터 없이 alpha 0 + GONE. 그대로인 것: 이동 없음 · 재표시 90ms 디바운스 · willHide→willShow 깜빡임 없음 · iOS 는 끝 프레임이 하단을 가릴 때만 키보드로 본다(하드웨어 · 플로팅 키보드 = 탭바 유지) · 오버레이 · 경로 · 오프라인 숨김 = D-12(150ms + 0.2s) · 문서 대기 = D-12b. 근거: iPhone 16(iPhone17,3) · iOS 26.6.1 운영 빌드에서 사용자 보고 「키보드가 나올 때 하단 탭바가 사라지는 게 늦어. 그냥 거의 즉시 사라지게 해줘」(Phase 21 UAT 3차 실서버 재확인 B1 후속). — **Reversibility:** reversible.
- **D-12a'' (2026-09-26 · quick-260926-vk9 · 사용자 실기기 재보고 — D-12 오버레이 숨김 타이밍 중 키패드 시트만 대체):** 키패드 시트(`NumberPadSheet` — 시스템 키보드 대신 뜨는 웹 숫자 키패드 · 터치 기기에서 수량·가격 칸이 연다)는 키보드 대체 입력이므로 D-12a' 를 따른다 — 열림 숨김 = 150ms 대기 · 0.2s 페이드 · 8pt/dp 하강 없이 **즉시**, 두 플랫폼 같음. 신호: 웹이 오버레이 계수 0→1 을 키패드 마커가 만들었을 때만 `overlay {open:true, immediate:true}` 를 보낸다 · 계수가 이미 1 이상이면 탭바가 이미 숨었으므로 추가 신호 없음. 닫힘(`open:false`)과 재표시는 D-12 그대로(지연 없이 0.2s · 키보드 90ms 디바운스 대상 아님). Sheet · Dialog · Popover 는 D-12 그대로(150ms 가 리다이렉트·짧은 오버레이 깜빡임을 삼킨다). 호환: 옛 네이티브는 필드를 무시해 D-12 · 새 네이티브 + 옛 웹은 필드가 없어 D-12. 근거: v5n(D-12a') 설치 뒤 iPhone 16(iPhone17,3) 운영 빌드에서 사용자 보고 「아직도 수량 입력칸 눌렀을때 탭바가 느리게 사라져」 — 수량 칸은 시스템 키보드가 아니라 키패드 시트를 열어 D-12 비키보드 경로를 탔다. — **Reversibility:** reversible.
- **D-12b (G-21-R3-4):** 전체 문서 로드 시작(iOS `didStartProvisionalNavigation` · Android `onPageStarted`) = 탭바 **대기** → 그 문서의 첫 `route` 메시지(하이드레이션 뒤 · 웹 계약 무변경) 또는 1.5초 뒤 280ms ease-out 표시 · SPA 이동은 무관 · iOS 도 숨긴 채 시작. 홈 데이터 로드까지 기다리는 안은 채택 안 함. — **Reversibility:** reversible. 근거 = 21-UAT.md G-21-R3-4.
- **D-25a (G-21-R3-2 — D-25 의 /trading 하단 공용 패널 항목 대체):** 앱(`html.native-app`)에서 /trading 공용 패널 + spacer 숨김(폰 · iPad 공통 · 브라우저 불변) · 카드 더티 바는 `--native-tabbar-offset` 까지 비킨다 · **전 종목 전략 로그는 /me — 스케치 008 ① B 채택: 전략 현황 카드 머리 「현황 | 로그」 알약 세그먼트 전환(기본 = 현황 · /me 세로 길이 불변 · D-20 순서 무변경)**. 잃는 동작(② 잔고 행 탭 → 그 종목 카드 생성·포커스 ③ 카드 없는 종목의 미체결 정정 진입이 앱에서 없어짐) = **사용자 동의 · 범위 밖**(21-25 답). — **Reversibility:** reversible. 근거 = 21-UAT.md G-21-R3-2.
- **D-29 (G-21-R3-8):** 뉴스 · 토론 전체 보기 = **탭 안 전체목록** — 종목상세 `?tab=news&view=news|discussions`(pushState · 허용 목록) · 트레이딩 ⓘ 팝업은 로컬 상태(URL 불변) · 뒤로가기(Android · 브라우저 · Esc · 화면 안 ←) = 요약 · **옛 URL `/stocks/{code}/news` · `/discussions` → `/stocks/{code}?tab=news&view=news|discussions` 리다이렉트로 유지(북마크 호환 · 21-25 사용자 동의)** · 당겨서 새로고침 = GET 재조회만(D-18). — **Reversibility:** reversible. 근거 = 21-UAT.md G-21-R3-8.
- **D-30 (G-21-R3-9):** 「주문하기」 → **「트레이딩」** = `/trading?code={code}` · 모든 폭 — 폰 하단 바(56 · r16 · 17/600 · `--up` 그대로, 라벨만) + **넓은 폭 = 스케치 008 ② A 채택: 히어로 첫 줄 끝 32 높이 알약(`--up` 채움 · 앱판은 「AI 분석」 옆)** · 매매 불가(비 KOSPI/KOSDAQ · isin 없음) 숨김 · 트레이딩은 그 종목 카드를 보장 · 펼침 · 포커스. — **Reversibility:** reversible. 근거 = 21-UAT.md G-21-R3-9.
- **D-31 (G-21-R3-10):** 종목상세 호가주문 탭 제거 · `?tab=orderbook` → `/trading?code=`(매매 불가면 차트) · 시간외종가 신규 주문 = 카드 수동주문 주문유형 — **스케치 008 ③ A 채택: 주문금액 위 44px 행 「주문유형 지정가 ›」(현재 호가주문 탭 UI 이동 · 투명 select)** · `affordanceOf` 그대로(창 닫힘 = 지정가 복귀 + 행 아래 안내) · 공유 모듈 유지 · 21-33 → 21-34 순서로 CTA 가 갈 곳 없는 중간 상태를 만들지 않는다. — **Reversibility:** reversible. 근거 = 21-UAT.md G-21-R3-10.
- **D-32 (G-21-R3-11 — D-06 의 탭 이동에 규칙 추가 · 2026-09-26 단순안):** 탭 루트(`/` · `/search` · `/trading` · `/chat` · `/me`) 사이를 오가면 탭 루트마다 **스크롤 위치를 복원**하고, 마지막으로 받은 내용을 **스켈레톤 없이 바로** 보여 준 뒤 조용히 갱신한다(기존 `lib/query-cache` 재사용 · 새 데이터 라이브러리 없음). 화면 재마운트와 입력 · 펼침 상태 초기화는 허용한다. 같은 탭 재탭 = 맨 위(D-06a) 유지. keep-alive(React Activity · 화면 보존)와 탭별 웹뷰는 기각(사용자: 과설계 금지). truth 는 이 범위로 좁힌다(21-UAT.md R3-11 decision 줄). — **Reversibility:** reversible. 근거 = 21-UAT.md G-21-R3-11.
- WR-03 Android 브리지 = 선택 a(21-25 사용자 답) — `androidx.webkit` 1.14.0 을 app 모듈에 선언(`variables.gradle` 의 `androidxWebkitVersion` 공유 · Capacitor 가 이미 APK 에 싣는 같은 좌표 · 런타임 의존 집합 불변) + `WebMessageListener` 로 허용 출처 · 메인 프레임만 수신(21-29 가 읽는다). 근거 = 21-REVIEW.md WR-03.

### Claude's Discretion
- `viewport-fit=cover` 는 웹·앱 공통 적용(D-25) — 헤더·하단 고정 요소가 safe-area 패딩을 갖추므로 iPhone 가로 Safari 에서도 노치 아래로 내용이 들어가지 않는다. 앱 전용으로 좁히는 대안은 기각(런타임 meta 교체 복잡도 > 이득).
- 키보드가 열린 동안(입력 포커스) 네이티브 탭바 숨김 — D-12 오버레이 규칙의 연장(양 플랫폼).
...
- 테마: `attribute="class"` `.dark` · `enableSystem=false` · 기본 light. `themeColor` 메타는 OS `prefers-color-scheme` 을 보는 별개 값(앱은 D-23 메시지 사용). (→ D-23a 기본 dark)
