...
- **D-12a (G-21-R3-1 — D-12 의 키보드 숨김 타이밍 대체):** 키보드 사유 숨김 = 150ms 대기 없이 **즉시** · 키보드보다 짧은 페이드(iOS = `userInfo` 애니메이션 시간의 절반 · 같은 곡선, Android = IME 애니메이션 `onPrepare` 에서 즉시) · 아래로 내려가는 이동 없음 · 재표시 90ms 디바운스 · iOS 는 키보드 끝 프레임이 화면 하단을 가릴 때만 키보드로 본다. 오버레이 · 경로 · 오프라인 숨김은 D-12 그대로. — **Reversibility:** reversible. 근거 = 21-UAT.md G-21-R3-1.
