...
updated: 2026-06-24T13:10:00Z
---

## Current Test

[완료 — 사용자 승인 2026-06-24]
...
result: passed — 사용자 승인 (2026-06-24). 초기 이슈(동반율 과소표기 WR-01, 섹션 sticky WR-04)는 GAP-1로 수정·재배포, 직접동반 점수 v2 재설계는 GAP-2로 적용, 표시 정렬을 실시간 등락률 순으로 변경(GAP-3)까지 반영 후 최종 승인.
...
status: resolved — production 재배포 + 사용자 승인 (2026-06-24)
...
status: resolved — production 적용 + server 재배포 + 사용자 승인 (2026-06-24)
...
### GAP-3: 동조 후보 표시 정렬 강도 → 실시간 등락률 순 (사용자 UAT 요청)
status: resolved — webapp 재배포 + 사용자 승인 (2026-06-24)
source: 사용자 UAT 요청 (2026-06-24) — "동반목록 정렬을 강도바 기준이 아니라 지금의 등락률 순으로."
fix: webapp stock-comovement-section.tsx — 선정(TOP-K)은 server strength 유지, 표시 순서만 client에서 liveChangeRate desc(null 맨 뒤). 헤더 캡션 "동반율 순 → 실시간 등락률 순". 회귀 테스트 Test 9 추가. (측정 방식 질문 "전체기간 유사도"는 사용자 결정 "현재 방식 유지" — v2 그대로.)
commits: 1178899 (정렬), 22b1a65 (DEPLOY-LOG)
deploy: webapp `gh-radar-webapp-29rx3mmct` Ready, prod alias 매핑 확인, 게이트 정상(307). 컴포넌트 테스트 9 + typecheck green.
