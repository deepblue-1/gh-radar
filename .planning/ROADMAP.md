...
- [x] 14-04-PLAN.md — Wave 2 chat-prompts(팀장+5전문가, 매매금지/환각금지/면책) + 데이터 전문가 4(결정적조회+Haiku 1콜) + 웹서치 전문가(web_search+citations)
- [x] 14-05-PLAN.md — Wave 3 chat-orchestrator: SPECIALIST_TOOLS 5종 + runSpecialist dispatch + extractStockRefs(D-07)
- [x] 14-06-PLAN.md — Wave 4 chat-service 이식(ww-bot: sanitize/prune/retry/캐싱/interrupt + 팀장 tool 루프 병렬) + routes/chat.ts(SSE POST + GET/DELETE) + app.ts 결선
- [x] 14-07-PLAN.md — Wave 2 프론트 기반: react-markdown 설치 + chat-sse(raw fetch+parseSSEStream+Bearer) + chat-api + chat-provider
- [x] 14-08-PLAN.md — Wave 3 FAB(로그인 게이트 D-01 + 종목 컨텍스트 D-03) + 챗 시트(SSE 스트리밍 + 정지/abort D-06) + composer + 상태박스 + layout/nav 마운트
...
- [x] 15-05-PLAN.md — relay 부팅 결선·graceful shutdown + 내부 HTTP `/healthz`(공유 비밀 상수시간 검증) + Dockerfile + 관리자 등록 스크립트 (SC-8)
...
- [x] 15-09-PLAN.md — 마이그레이션 3건(`stocks.isin`·`dma_credentials`·`dma_orders`) + [BLOCKING] `supabase db push` (SC-4, SC-6)
- [x] 15-10-PLAN.md — ISIN 매핑 파이프라인(shared·master-sync·server 노출) + 백필 + `smoke-relay.sh --check-isin` (SC-6)
...
- [x] 15-19-PLAN.md — `deploy-server.sh` relay env + 재배포·결선 검증 + **mock 브로커 가격 0 거부 실측** (SC-6, SC-8)

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 15-20-PLAN.md — [BLOCKING] 실서버·실계좌 검증 여부 **사용자 결정**(D-27) + SC-1~8 집계 + STATE 갱신 (SC-8)
...
- [x] 16-01-PLAN.md — REQUIREMENTS 등록 · relay 생성코드 재동기화 · shadcn checkbox · dma_orders.origin 마이그레이션 [BLOCKING db push]

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 16-02-PLAN.md — 테스트 하네스: FakeGateway 전략 프레임 주입 · LocalRelay 노출 · Playwright 8090 직렬화
- [x] 16-03-PLAN.md — shared 전략·주문 wss 계약 + relay protocol zod 인바운드 6종

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 16-04-PLAN.md — msg-type 화이트리스트 확장(+14) + envelope 전략 요청 빌더 7종

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 16-05-PLAN.md — envelope 전략 응답 파서 6종(60/61/64/65/72/73/56) + 왕복 테스트

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 16-06-PLAN.md — SubscriptionHub 전략 캐시 3맵 + Ready 프리페치(24/21/34) + 프레임 case 7종

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 16-07-PLAN.md — fanout 전략 인바운드 4종(계좌 화이트리스트) + auth 직후 전략 스냅샷 팬아웃

**Wave 7** *(blocked on Wave 6 completion)*

- [x] 16-08-PLAN.md — 주문 wss 이관(5초 상관) + SymbolMap market + dma_orders insert/origin(자동주문 분기)

**Wave 8** *(blocked on Wave 7 completion)*

- [x] 16-09-PLAN.md — RelayProvider 전역 승격 + useRelaySubscription(구독 ref-count) + orderbook 회귀
...
- [x] 16-12-PLAN.md — 상따 폼(스위치 즉시·값은 수정 버튼·둘 다 OFF=삭제) + dirty-action-bar + 순수 함수

**Wave 12** *(blocked on Wave 11 completion)*

- [x] 16-13-PLAN.md — 상따 페이지 조립(호가 10단+최근체결·전략 로그·에코 배너) + E2E
- [x] 16-14-PLAN.md — VI 페이지(설정·시작/중지 확인·주문내역·데드라인·마감알림) + E2E
- [x] 16-15-PLAN.md — My page(전략 현황·전체 비활성화·계좌별 미체결/잔고 반복) + E2E

**Wave 13** *(blocked on Wave 12 completion)*

- [x] 16-16-PLAN.md — server POST /api/orders 제거 · relay-client 삭제 · config/deploy env 정리

**Wave 14** *(blocked on Wave 13 completion)*

- [x] 16-17-PLAN.md — a11y 3표면 + 전체 E2E + 배포(relay→server→Vercel) + smoke + STATE/ROADMAP

**Wave 15** *(갭 클로징 — 16-VERIFICATION 갭 4건 + 16-REVIEW Critical 1·Warning 9)*
...
- [x] 16-21-PLAN.md — [gap 4 해법 ③] /healthz degraded 판정에서 never-Ready 세션 제외 + smoke INV-9 를 wss 주문 왕복으로 교체

**Wave 16** *(blocked on Wave 15 completion)*

- [x] 16-22-PLAN.md — [gap 2 + WR-02] 통보 상관 다축 매칭(좁히기 실패 시 미정산) + 사용자 스코프 중복 가드
- [x] 16-23-PLAN.md — [CR-01 + WR-08] 계좌축 소비자 정리(account 필드 제거·accountStates) + ISIN 역매핑 공용 훅
- [x] 16-24-PLAN.md — [WR-07 + WR-09] VI 주문금액 상한 3층 + flushNow 진행 중 대기·재큐잉 소진

**Wave 17** *(blocked on Wave 16 completion)*

- [x] 16-25-PLAN.md — [WR-03 + WR-06] lc.set 시장구분 소유권 relay 이전(D-28) + 발주가·수량 0 무장 차단
...
- [x] 16-27-PLAN.md — [GC-CR-01 + GC-CR-03] 통보 하드 필터(후보 1건 지름길 제거) + await insertRequest 후 연결 생존 재확인
- [x] 16-28-PLAN.md — [GC-WR-08 + GC-IN-04] 부분 UNIQUE 위반(23505) 재조회 수렴 + flushNow 라운드 상한 근거 정합
- [x] 16-29-PLAN.md — [GC-WR-04 + GC-WR-05] lc.set 삭제는 시장 해석 실패로 막지 않음 + relay 무장 가드를 UI canArm* 3식과 동형화
- [x] 16-30-PLAN.md — [GC-WR-07 + GC-WR-11] /healthz stalledCount 판정(재시작 후 상시 초록 방지) + smoke INV-9 env 토큰·verdict 덮어쓰기
- [x] 16-31-PLAN.md — [GC-WR-09 + GC-WR-12 + GC-IN-01 + GC-IN-02 + GC-WR-06 상따] handleSubmit 무장 가드 · 원인별 안내 문구 · send 반환값 분기 · 타입 서술자
- [x] 16-32-PLAN.md — [GC-WR-06 VI + GC-IN-03] vi.confirm·vi.set 전송 실패 시 낙관 반영·잠금 금지 + latestAccountTime 정규화 전 비교

**Wave 20** *(blocked on Wave 19 completion)*

- [x] 16-33-PLAN.md — [GC-CR-02 + GC-WR-01 + GC-WR-02] 수동 통보 조회 경유·0행 error 로그·stdout 감사 사본 + recordUnmatched .catch + 빈 주문번호 in-flight 제외

**Wave 21** *(blocked on Wave 20 completion)*

- [x] 16-34-PLAN.md — [GC-WR-03 + GC-WR-10] narrowPending 매매구분 축(sideTrusted) + 취소 중복 키를 원주문번호로 분리
...
- [x] 16-39-PLAN.md — [R2-WR-01 + R2-IN-04] `23505` 갱신 예외가 패치 전체를 버림 → `order_no` 만 빼고 1회 재시도 + `flushed` 정합 + 테스트 ⓽ 가 「행이 실제로 갱신됐는가」를 본다

**Wave 25** *(blocked on Wave 24 completion — `orders.ts` 순차)*

- [x] 16-40-PLAN.md — [R2-WR-07 + R2-WR-04] `inserted` 가 23505 수렴을 셈 · 수렴 재조회 예외가 원래 사유를 덮음 · `flushNow` 가 진행 중 배치를 덮어씀
...
- [x] 16-42-PLAN.md — [갭 4 webapp + R2-WR-02 + R2-IN-01] `isin-labels` 원천에 상따 전략 추가 + 철거 의도의 「수정」 무장 가드 면제(relay 와 동형) + 안전 문구 자동 해제
- [x] 16-44-PLAN.md — [R2-WR-05] `WsFanout#register` 의 세션 `"state"` 리스너 누수 — entry 가 핸들을 소유하고 `#register`·`#onClose` 양쪽이 뗀다
...
**잔여 (phase 를 닫으려면 이 둘):** ① D-25 mock 게이트웨이 실기 검증 — `sudo xcodebuild -license` 동의 후 `cd /Users/alex/repos/gh-trade/server && ./scripts/build.sh --server-only` 로 HEAD 재빌드해야 76/77/78 드롭 0 과 36/37/38 래치 왕복을 볼 수 있다(WINDOWS #17, 배포로 닫히지 않는다). ② **relay 배포 완결** — webapp 은 이미 나갔고 relay 만 남았다. `DMA_HOST` **미주입**으로 `bash scripts/deploy-relay.sh` → `smoke-relay.sh` → `/healthz`. 되돌리려면 webapp 을 `gh-radar-webapp-a7b0evnin` 로 롤백.
...
- [x] 17-03-PLAN.md — 신규 푸시 3종(76/77/78) 중계: 파서·세션 캐시·인증 스냅샷·드롭 0 게이트 (wave 3)
- [x] 17-08-PLAN.md — 체결테이프 서버 체결구분 · 호가 종목정보 종가 · 호가 상태바 출처 배지 (wave 3)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 17-04-PLAN.md — 래치 수동 점등 `lc.arm` → ArmSell/Cancel/BuyLatchReq 왕복 + 가드 (wave 4)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 17-05-PLAN.md — VI 거래소 축(relay): 21 KRX/NXT 2회·요청 거래소 FIFO·거래소별 캐시·`{t:"vi",x}` (wave 5)

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 17-06-PLAN.md — VI 웹 표면: 거래소별 리듀서·가동 배지 합집합·캡션 정정·거래소 열 (wave 6)

**Wave 7** *(blocked on Wave 6 completion)*

- [x] 17-09-PLAN.md — 미체결 표식 헬퍼 · 취소보관 회색/취소 숨김 · 예약·접수대기 문구 표시 (wave 7)
- [x] 17-10-PLAN.md — 주문통보 행위 단어·수동·시간외종가 + 자동주문 3초 창 묶기 (wave 7)
- [x] 17-11-PLAN.md — 상따 상태줄 LED 결선·`lc.arm` 전송 · 전략 로그 래치 전이 4종 · 헤더 하한 칸 종가 대체 (wave 7)
...
- [x] 18-01-PLAN.md — 계약 수직 슬라이스(tracer): 정정 `order.modify` end-to-end · `pieceCount`/`krxSession` 조건부 송신 · `price 0` 4겹 조건부 완화 · 돌파 `name`/`code` relay 보강(D-30) (wave 1)
- [x] 18-02-PLAN.md — 디자인 토큰 `--new-bg`/`--new-bd` 승격 · globals.css §2.2b 컨테이너 문단 · `parseStrategyKey` 를 `lib/limit-chaser.ts` 로 이동 (wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 18-03-PLAN.md — [BLOCKING] Supabase 스키마 푸시 + 실 DB 제약 조회 검증 (wave 2)
...
- [x] 18-05-PLAN.md — VI 설정 2줄(KRX/NXT, 고정 거래소 상수 폐기) · VI 발동 스트립/표 (wave 2)
- [x] 18-06-PLAN.md — 카드 골격: `quote-grid-10` · `card-header` · `strategy-card`(`@container/lc` 선언 이동) (wave 2)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 18-07-PLAN.md — 수동주문 폼(4버튼·적응형 진입·정정/취소 선택) + 주문확인 다이얼로그 확장 (wave 3)
- [x] 18-08-PLAN.md — 돌파감지 스트립/표(클릭 → 카드 추가) · 종목 추가 검색란 (wave 3)
- [x] 18-09-PLAN.md — 공용 패널(미체결·잔고·로그) · 종목정보 팝업 (wave 3)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 18-10-PLAN.md — 카드 본문 조립 · 더티 힌트 prop 화 · 종목상세 호가 탭 통일 (wave 4)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 18-11-PLAN.md — 작업대 셸 `/trading` · 상태줄 · 카드 격자(펼침 우선 + 접힘 스택) (wave 5)

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 18-12-PLAN.md — 리다이렉트 4경로 · 사이드바 재구성(제목 링크 + 3단) · 링크 지점 정리 (wave 6)
...
- [x] 18-14-PLAN.md — [CR-01 tracer] 시간외종가(가격 0) 원주문 취소 — webapp 번역기·relay zod·조립기 한 규칙 + 마이그레이션 파일 + 층별 회귀 (wave 8)
- [x] 18-15-PLAN.md — [CR-02] VI 줄 계좌 정본 = 등록 전략 계좌(`viRowAccountOf`) + 불일치 고지 + 확인 요약 정본 계좌 (wave 8)
- [x] 18-16-PLAN.md — [WR-01 + WR-06] 시간외종가 세션 없음 → 주문 안 만듦 · 정정 수량 잔량 추종/검증/재대조 · 가격 0 원주문 「시간외종가」 표기 (wave 8)

**Wave 9** *(blocked on Wave 8 completion)*

- [x] 18-17-PLAN.md — [사용자 결정 2026-09-22] 돌파 목록 최신 돌파가 맨 위 — relay getter·78 팬아웃·웹 리듀서 한 축 + e2e GC1 (wave 9)
- [x] 18-18-PLAN.md — [CR-01 · BLOCKING db push · 한쪽 문 체크포인트] 원격 `dma_orders_price_check` 취소 가격 0 허용 + 전후 덤프 diff (wave 9)
- [x] 18-19-PLAN.md — [WR-03 확인 우선] 취소·정정 대기 요청 종류 재현 → (재현 시) `PendingOrder.kind` 하드 필터 (wave 9)

**Wave 10** *(blocked on Wave 9 completion)*

- [x] 18-20-PLAN.md — [WR-02] 카드 접기/펴기 재마운트 제거 — 카드별 고정 호스트 노드 + 한 번 펼친 본문 유지 + e2e GC2 (wave 10)

**Wave 11** *(blocked on Wave 10 completion)*

- [x] 18-21-PLAN.md — [WR-05 확인 우선] 같은 종목 두 번째 전략 은닉 재현 → (재현 시) 카드 정체성 ISIN → 카드 id · 전략 키 기준 유입/포커스 + e2e GC3 (wave 11)

**Wave 12** *(blocked on Wave 11 completion)*

- [x] 18-22-PLAN.md — [WR-04 + WR-07 확인 우선] 미체결 선택 → 받을 카드 보장 + e2e GC4 · 포커스 보류는 첫 64 스냅샷 전에만(`limitChaserSnapSeq`) (wave 12)
...
- [x] 18-25-PLAN.md — [GC-WR-01 + GC-IN-01 relay] 통보 `requestKind` 정본(E+Modify 정정 정산) · 모순 통보 0건 · `refersOrg` 개명 · 옛 주석 정정 (wave 14)
- [x] 18-26-PLAN.md — [GC-IN-02 + deferred] relay `#limitChaserKnown`(64 받았을 때만 인증 `lc.snap`) · 작업대 `knowsRegistered = snapSeq > 0` · ready 전환 기준점 (wave 14)
- [x] 18-27-PLAN.md — [GC-WR-02 + GC-IN-06 + GC-IN-03] 취소 확정 = 현재 잔량으로 내림(폼·계좌 패널) · 재대조 문구 분리 · `isOffhoursOrder` 단일 판정 (wave 14)
- [x] 18-28-PLAN.md — [GC-WR-04 사용자 결정] 중지 VI 만 「상태줄 계좌({A})로 옮겨 시작」 · 요약 「계좌 B → A」 · 가동 중 계좌 고정 유지 (wave 14)

**Wave 15** *(blocked on Wave 14 completion)*

- [x] 18-29-PLAN.md — [GC-CR-01 · BLOCKING db push · 운영 DB 체크포인트] 위반 행 읽기 전용 사전 확인 → null-safe CHECK 원격 반영 + 전후 덤프 diff (wave 15)
- [x] 18-30-PLAN.md — [GC-WR-03] 「결과 모름」 잠금을 작업대 키 상태로 · 잠긴 카드 ✕ 확인 다이얼로그 · 재추가 3경로 잠금 유지 + e2e GC5 (wave 15)
...
- [x] 18-32-PLAN.md — 갭 클로징 R3 전량 게이트 · 18-VALIDATION §Gap Closure R3(12행 · 대응표 · 정정 기록 · 배포 순서 R3) · REQUIREMENTS Pending 유지 · deferred 해소 (wave 17)

**Wave 18** *(갭 클로징 R4 — 18-REVIEW-R3 7건: R3-WR-01 · R3-WR-02 · R3-IN-01~05 + 18-VERIFICATION-R3 escalation 2건 · 사용자 결정 2건)*

- [x] 18-33-PLAN.md — [R3-WR-01 tracer] relay 상태 단조성 — 늦은 정정확인 M 이 체결된 정정 행 · 원주문 행을 되돌리지 않는 조건부 UPDATE(`replaceableStatusesOf`) · ws ㊹ ㊺ RED → GREEN · 49칸 전이표 (wave 18)
- [x] 18-34-PLAN.md — [R3-WR-02 + R3-IN-01 + R3-IN-02 tracer · 사용자 결정] 결과 모름 잠금을 `RelayProvider`(앱 수명)로 · 신규 · 정정만 등록 · 진행 중 포함 · 호가 탭 공유 · ✕ 본문 · e2e GC6 · CONTEXT D-27 보강 (wave 18)
...
- [x] 20-02-PLAN.md — Wave 0 순수 기반: krxTick(shared · 호가 단위 한 곳) · numpad 키패드 규칙 · 폭 예산 스파이크

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 20-03-PLAN.md — 터치 슬라이스: useEditMode · 토큰 3개 · NumberPadSheet(body 포털 · 440 가운데) · 트레이서 행 시트 연결(hasTouch e2e)
...
- [x] 20-06-PLAN.md — 수동주문 토스 스타일(상자 · 48/38 버튼 · 스테퍼 제거) + 키패드 시트 입력(값만 채움)
...
**Depends on:** Phase 20
**Plans:** 35/36 plans executed (갭 클로징 21-17~21-24 추가 2026-09-26 — 21-UAT.md G-21-1 · G-21-N1~N3 · UAT 3차 갭 클로징 21-25~21-36 추가 2026-09-26 — G-21-R3-1~11 · G-21-CR · 같은 날 과설계 정리로 18→12 플랜 · R3-11 단순안)
...
- [x] 21-01-PLAN.md — (W1) D-20 번들 ID 게이트 · 패키지 정당성 게이트 · 트레이서: `mobile/` + cap add ios + GHTradeBridgeViewController + 웹 감지 스크립트 → `ready` 가 시뮬레이터 로그에 도착

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 21-02-PLAN.md — (W2) Android 셸 슬라이스: cap add android · Kotlin MainActivity · BridgeWebViewClient 상속 · GhTradeBridge · 에뮬레이터 스모크
- [x] 21-03-PLAN.md — (W2) Google OAuth 콘솔 선행(GCP iOS·Android 클라이언트 · Supabase Authorized Client IDs) + 공개 ID 상수
- [x] 21-04-PLAN.md — (W2) 웹 네이티브 브리지 코어: postNative · NativeBridgeProvider(window.__ghTrade · refresh 레지스트리 · route/theme/pull) · 오버레이 마커

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 21-05-PLAN.md — (W3) 앱 셸 분기(D-09 사이드바 · D-10 「AI 분석」) + e2e 앱 모드 픽스처 · native-shell.spec
- [x] 21-07-PLAN.md — (W3) 새로고침 연결: relay `probeNow`(D-16) · 목록 4화면 · 트레이딩/마이 · 종목상세 GET 만(D-18)
- [x] 21-10-PLAN.md — (W3) iOS 탭바(D-27a 알약) · 경로표(D-14) · 숨김 규칙 · 탭 → navigate 훅
- [x] 21-14-PLAN.md — (W3) 아이콘·스플래시(D-22 `#app-a`) 렌더 · @capacitor/assets 일회 생성 · 웹 파비콘
...
- [x] 21-09-PLAN.md — (W5) 브랜드명 GH Trade(D-21) + `/me` 계정 카드 A(D-08a)
- [x] 21-13-PLAN.md — (W5) Android SwipeRefresh · onBackPressedDispatcher 뒤로가기 · 테마 · 방향 · 오프라인

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 21-15-PLAN.md — (W6) 네이티브 Google 로그인(D-03): nonce · signInWithIdToken · /login 분기 · capgo 플러그인 설치·설정

**Wave 7** *(blocked on Wave 6 completion)*

- [x] 21-16-PLAN.md — (W7) 운영 설정 검사 · mobile/README · 전 게이트 · 실기 UAT + push(웹 배포) 승인 체크포인트

**Wave 8** *(갭 클로징 — 21-UAT.md G-21-1 · G-21-N1 · G-21-N2 · G-21-N3, 사용자 결정 2026-09-26 · G-21-13 은 resolved 로 플랜 없음)*
...
- [x] 21-21-PLAN.md — [G-21-1 Android] 탭바 C 근사(라벨 없음 · 캡슐 · 불투명 유리 · elevation 12) · 높이 60dp · 다크 대비 계산

**Wave 9** *(blocked on Wave 8 completion)*

- [x] 21-22-PLAN.md — [G-21-N3 iOS] 호스트 밖 http(s) = SFSafariViewController(decidePolicyFor · createWebViewWith) · 같은 호스트 새 창은 WebView 유지 · Android 와 같은 판정 표(swiftc)

**Wave 10** *(blocked on Wave 9 completion)*

- [x] 21-23-PLAN.md — 갭 클로징 전 자동 게이트 · push 전 UAT 환경 준비(dev 웹 · UAT 프록시 · 세 기기 새 설치)

**Wave 11** *(blocked on Wave 10 completion)*

- [x] 21-24-PLAN.md — UAT 재검증 + push(웹 배포) 결정 체크포인트 · 운영 sync 복원 · (선택 시) push → 운영 반영 확인 → 운영 빌드 재설치

**Wave 12** *(UAT 3차 갭 클로징 — 21-UAT.md G-21-R3-1~11 · G-21-CR, 사용자 결정 2026-09-26)*

- [x] 21-25-PLAN.md — 목업 게이트: 스케치 008(/me 전 종목 전략 로그 · 종목상세 넓은 폭 「트레이딩」 · 카드 주문유형) 검토 + 재확인 3건(잃는 동작 · 옛 URL 리다이렉트 · WR-03 androidx.webkit) → CONTEXT D-12a · D-12b · D-25a · D-29~D-32(R3-11 축소)
- [x] 21-26-PLAN.md — [G-21-R3-3 · R3-5 · R3-6 · R3-7] 만원 칩 천만/오천만/1억 · 가격 섹션 pt-1 · 카드 ✕ 32/히트 44 · 종목정보 팝업 고정 높이 · ✕ · safe-area
- [x] 21-27-PLAN.md — [G-21-CR WR-01 · IN-07 · WR-05 · WR-04 · IN-06] isSafeInternalPath 공유 · 최근 검색 코드 형식 · 검색 resultsQuery · 팝오버 오버레이 마커 · theme-color = 앱 테마
- [x] 21-28-PLAN.md — [G-21-R3-1 · R3-4 iOS · WR-03 iOS · IN-02 · IN-08 · IN-05] 키보드 즉시 숨김 · 문서 로드 대기 · 출처 전체 비교 · 폴백 dev=1 · 스모크 기기 지정 · README 명령표

**Wave 13** *(blocked on Wave 12 completion)*

- [x] 21-29-PLAN.md — [G-21-R3-1 · R3-4 Android · IN-01 · WR-03 · WR-02 · IN-02 · IN-03] IME 애니메이션 콜백 · onPageStarted 대기 · pullBlocked 초기화 · 브리지(21-25 답) · 백업 끄기 · 폴백 페이지 dev=1 · 오프라인 뒤로가기 재현
- [x] 21-30-PLAN.md — [G-21-R3-8 종목상세] 탭 안 전체목록(NewsFullList · DiscussionFullList · ?view=) · 뒤로가기 네 경로 · 탭 재클릭 = 요약 · 옛 URL 리다이렉트
- [x] 21-31-PLAN.md — [G-21-R3-11 · D-32] 탭 루트 스크롤 복원(useTabRootScrollMemory) · 기존 query-cache 로 스켈레톤 없는 재방문(검색 허브 · 대화 목록 · 오늘 주문)
- [x] 21-32-PLAN.md — [G-21-R3-2] 앱 공용 패널 숨김 · 더티 바 탭바 비킴 · StrategyLogFeedProvider · /me 전 종목 로그(채택안)
...
- [x] 21-34-PLAN.md — [G-21-R3-9 · R3-10 종목상세 · IN-04] 「트레이딩」 CTA(폰 바 + 넓은 폭) → 호가주문 탭 제거 · 딥링크 · 탭 전용 코드 삭제 · orderbook.spec 선별 이전 · variant 제거

**Wave 16** *(blocked on Wave 15 completion)*

- [x] 21-35-PLAN.md — 전 자동 게이트(worktree 프로덕션 빌드 포함) · 21-REVIEW-R2.md(13건 대조) · push 전 UAT 환경

**Wave 17** *(blocked on Wave 16 completion)*

- [ ] 21-36-PLAN.md — UAT 3차 재검증 + push(웹 배포) 결정 체크포인트 · 운영 sync 복원 · (선택 시) push → 운영 반영 확인 → 운영 빌드 재설치
