...
Plans:
- [ ] 11-01-PLAN.md — Wave 0 스캐폴드: COMV-01 등록 + 사전계산 마이그레이션(theme_comovement/cosurge_edges + 부분인덱스 + rebuild_comovement RPC) + 공유 타입 + RED 테스트 + co-movement-sync 워크스페이스
- [ ] 11-02-PLAN.md — Wave 1 [BLOCKING] supabase db push + rebuild 실행 + fixture co_count 대조 + EXPLAIN → task-timeout 확정
- [ ] 11-03-PLAN.md — Wave 2 읽기 경로: computeComovement 순수함수 + GET /api/stocks/:code/co-movement 라우트(themes.ts 청크 IN) + server 재배포 + prod curl
- [ ] 11-04-PLAN.md — Wave 2 워커: co-movement-sync(rebuild RPC 1줄) + IAM/deploy/smoke + Cloud Run Job + Scheduler(야간 1회)
...
| 11. Co-movement Candidates | 0/5 | Planned | — |
