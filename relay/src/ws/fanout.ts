...
export class WsFanout {
  ...
  async #onFirstMessage(conn: Conn, msg: RelayInbound): Promise<void> {
    ...
    //
    // ⚠️ `lc.snap` 은 hub 가 게이트웨이 64 를 **받았을 때만** 캐시에서 내린다 (18-26 /
    //    GC-IN-02) — 그때의 빈 배열은 「등록 전략 없음」의 확정 정보다. 모르면(콜드 세션 ·
    //    세션 교체 직후) **보내지 않고**, 곧 오는 64 팬아웃이 이 연결의 첫 `lc.snap` 이 된다.
    //    VI 설정의 3상태 규율과 같다. 빈 캐시를 `lc.snap []` 으로 내리면 브라우저가 「모름」을
    //    「없음」으로 읽고 포커스 보류를 거짓 확정 위에서 판정한다.
    // ⚠️ `vi.list` 는 **비어 있어도 1프레임 보낸다** — 안 보내면 브라우저는 「아직 안 왔다」와
    //    구분하지 못해 계속 스켈레톤이다. (이 규율은 18-26 에서 바꾸지 않았다 — `vi.list`
    //    에는 「모름」 으로 판정을 내리는 소비처가 없다.)
    // ⚠️ VI 설정은 `getViTrigger` 3상태 — `undefined`(61 을 아직 못 받았다)면 **보내지
    //    않는다.** 지어낸 「미등록」을 내리면 브라우저가 사용자가 입력 중인 금액을 지운다
    //    (16-06 결정 2). `null`(조회 결과 미등록)은 확정이므로 보낸다.
    if (this.#hub.hasLimitChaserList(userId)) {
    ...
    // ⚠️ `rate.cross.snap` 은 `vi.list` 와 같다 — **비어 있어도 1프레임 보낸다.** (18-26 의
    //    `lc.snap` 게이트를 여기 넓히지 않는다 — 76 은 로그인 전 연결에도 오고, 이 결함과 무관하다.)
  ...
  #onAuthedMessage(conn: Conn, userId: string, msg: RelayInbound): void {
    ...
    // ------------------------------------------------------------------
    // 주문 3종 (D-02 · 정정은 Phase 18 D-21). 전략과 **같은 자리**다 — `isin` 은 있지만 `ex`/`isin` 조합으로
    ...
    // ------------------------------------------------------------------
    if (msg.t === "order.new" || msg.t === "order.modify" || msg.t === "order.cancel") {
      if (this.#orders === null) {
