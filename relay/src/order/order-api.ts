...
// 판정은 wss 경로(16-08)와 **같은 함수**를 쓴다 — 두 벌이 되는 순간 한쪽만 고쳐진다.
import { ORDER_RESP_TIMEOUT_MS, filledQtyOf, statusOf } from "./notice-status.js";
...
/**
 * 첫 통보 대기 상한 재export — 정본은 `order/notice-status.ts` 다 (D-02).
 *
 * wss 주문 핸들러(16-08)와 **같은 값**이어야 하므로 두 곳에 적지 않는다. 이 파일은
 * 16-16 에서 지워지고 상수는 중립 모듈에 남는다.
 */
export { ORDER_RESP_TIMEOUT_MS } from "./notice-status.js";
