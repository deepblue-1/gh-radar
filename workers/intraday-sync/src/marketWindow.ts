...
//
// 배경: intraday-sync 는 Scheduler cron `* 8-19 * * 1-5` + 보조 `0-2 20 * * 1-5` (KST) 로 08:00~20:02 매분 돈다.
//   그런데 키움 ka10027 은 stex_tp="3"(통합) 이라 KRX 정규장 밖에서는 NXT 프리마켓
//   (08:00~08:50) / NXT 애프터마켓(15:40~20:00) 체결가를 cur_prc 로 돌려준다.
 ...
 *   정규장 데이터다. 15:31 부터는 정규장 밖이다(NXT 애프터마켓 15:40~ 개시 전후 — 보수적으로 창 밖 처리).
 */
 ...
 * 상한 15:55 — **16:00~20:00 KRX 애프터마켓(접속매매, 가격제한폭 전일종가 ±30%) 개시** — KRX 전용
 *   cur_prc 도 다시 움직이므로 EOD 종가 패스는 16:00 이후 금지.
