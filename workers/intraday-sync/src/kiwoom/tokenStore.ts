 ...
 * RESEARCH §5.3 + CONTEXT D-26/D-27.
 *
 * race condition idempotent (RESEARCH §5.4) — upsert onConflict token_type DO UPDATE.
 * worker + server 가 동일 row 공유 (D-18, D-26).
 *
 * Flow:
...
export async function getKiwoomToken(
  ...
  // 3. UPSERT (race condition idempotent — 마지막 INSERT 승, D-27)
