 ...
 *
 * RESEARCH §Pattern 6:
 *  - (a) 발굴: 최근 뉴스 제목+설명 → 기존 시스템 테마에 없는 신규 테마/이슈 키워드 + 관련 종목코드 (JSON).
...
- 각 테마에 해당 뉴스에서 관련된 종목코드(6자리)를 함께 제시한다. 종목코드가 불확실하면 비운다.
- confidence 는 0~1 (이 테마가 실제 의미있는 신규 테마일 확신도).
- 애매하거나 잡음(욕설·감탄·광고)뿐이면 아무것도 발굴하지 않는다(빈 배열).

출력은 반드시 JSON 만. 다른 텍스트 금지:
{"themes":[{"name":"테마명","stockCodes":["005930","000660"],"confidence":0.8}]}
발굴할 것이 없으면 {"themes":[]}.`;
...
뉴스:
- 초전도체 관련주 일제히 급등… LK-99 재현 기대감
- 상온 초전도체 테마 연일 강세, 관련 종목 상한가
- 삼성전자 3분기 영업이익 발표`,
    assistant: `{"themes":[{"name":"초전도체","stockCodes":[],"confidence":0.85}]}`,
  },
  {
    user: `EXISTING: 반도체
뉴스:
