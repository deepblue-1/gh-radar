...
/**
 * prevThemes 출처 (quick-260915-boq) — "slot": 오늘 직전 슬롯 스냅샷,
 * "prevTradingDay": 오늘 테마가 비어 이월한 전 거래일 마지막 non-empty 스냅샷.
 */
export type PrevThemesSource = "slot" | "prevTradingDay";
...
- 직전 테마 구성(직전 슬롯 또는 전 거래일 마지막)이 주어지면 그것을 **기본값으로 유지**하라. 새 뉴스나 급등 집합 변화가 명확히 다른 분류를 요구할 때만 변경한다. 같은 묶음의 테마명은 직전 이름을 그대로 재사용하고 새 이름을 붙이지 마라(슬롯 간 명멸 방지).
 ...
 *
 * prevThemes (quick-260720-kyh sticky prior): 직전 슬롯 스냅샷의 테마 구성. 비어있으면
 ...
 * prevThemesSource (quick-260915-boq): "slot"(기본, 오늘 직전 슬롯) | "prevTradingDay"(오늘 테마가
 * 비어 전 거래일 마지막 테마를 이월) — 섹션 헤더 라벨만 분기, 멤버 필터는 동일.
 */
export function formatClusterMessage(
  ...
  if (prevThemes.length > 0) {
    ...
    if (prevLines.length > 0) {
      const label =
        prevThemesSource === "prevTradingDay" ? "전 거래일 마지막" : "직전 슬롯";
      message += `\n\n직전 테마 구성 (${label}):\n${prevLines.join("\n")}`;
    }
  }
