  ...
  return (
    <div aria-label="관심종목 로딩 중" aria-busy="true" role="status">
      ...
      <div className="hidden lg:block overflow-hidden rounded-[var(--r-md)] border border-transparent bg-[var(--card)]">
        <div
          className={
            'grid grid-cols-[1fr_100px_80px_120px_100px_140px_44px] items-center gap-3 px-3 bg-transparent py-2 text-[length:var(--t-caption)] font-semibold text-[var(--muted-fg)] uppercase tracking-wide'
          }
        >
          <span>종목명</span>
          <span>코드</span>
      ...
      <div className="lg:hidden flex flex-col gap-2 skeleton-list">
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            key={i}
            className="flex items-center gap-3 rounded-lg border border-transparent bg-[var(--card)] p-3"
          >
