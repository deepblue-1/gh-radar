 ...
 *     · 2단 호가(700~829)      — `가격 | 잔량` 2열 · 마커 슬롯 없음
 *     · 1단 사다리(~699)       — 2줄 행 · 340px 스크롤
 *   숨김은 Tailwind `hidden`(=`display:none`)이라 **접근성 트리에서도 빠진다**.
 * ★ 그리고 트리 **밖**에 compact 체결 테이프 **1벌**(`ladder-tape` · 본문 830 미만에서만 보인다 ·
 *   quick-260923-elb 3a). 3단 표는 체결 10건을 품지만 2단·1단에는 그 칸이 없어 이 테이프가
 *   대신한다. 옛 구조는 두 트리가 테이프를 한 벌씩 들고 있었고, 숨은 쪽까지 매 커밋 200행을
 *   재조정했다(debug `trading-cpu-260923`).
 * ★ 1단 사다리는 (260911-w5h):
  ...
  */
  ...
  return (
    ...
    >
        ...
        */}
        {/*
          ...
          ★ `tabIndex={0}` 은 장식이 아니다. 박스 안에 포커스 가능한 자식이 하나도 없어서,
            ...
            달지 않는다.**
          ★ 가로선과 체결 테이프는 이 박스 **밖**이다 — 트리 밖 공용 블록(`ladder-tape`)이다.
            체결 10건은 스크롤에 묻히지 않는다.
        */}
        <div
          ref={scrollTwoRef}
          tabIndex={0}
      ...
      {/* ── compact 체결 테이프 (본문 ~829) — 트리 밖 1벌 · 2단 호가와 1단 사다리가 공유 ── */}
      {/*
        ★ quick-260923-elb 3a — 2단·1단 트리가 한 벌씩 들고 있던 테이프를 **트리 밖 1벌**로
          합쳤다. 숨은 트리의 테이프까지 매 커밋 200행을 재조정하던 비용이 절반이 된다.
        ★ 시각적으로 이전과 같은 이유: 트리는 자기 밴드 밖에서 `display:none` 이라 공간을
          차지하지 않는다. 그래서 1단(~699)에서는 「1단 사다리 → 가로선 → 테이프」, 2단
          (700~829)에서는 「2단 박스 → 가로선 → 테이프」로 이전과 같은 순서다. 830 이상에서는
          숨겨지고 3단 표의 최근 체결 10건이 대신한다. 경계 830 은 2단·3단 트리가 이미 쓰는
          컨테이너 쿼리 클래스 그대로다(정본 globals.css §2.2b — JS 폭 판정을 두지 않는다).
        ★ 덤으로 1단↔2단 밴드를 오가도 테이프 인스턴스 하나가 핀·스크롤 상태를 유지한다.
        ★ 제목행도 컬럼헤더도 두지 않는다(사용자 확정).
        ★ 가로선에 `border-0` 을 함께 쓰는 이유는 UA 기본 테두리가 남아 이중선이 되기 때문이다.
          굵기·색은 1단 사다리의 매수1 경계선과 **같다**.
        ★ props 는 전부 **이 컴포넌트가 이미 들고 있는 값**이다 — 체결내역을 위해 만든 새 조회
          경로가 0개다.
      */}
