 ...
 *
 * ⑥ 우측 여백은 **공용 컴포넌트의 방어**로 남긴다 (16-13 실측 결함 → quick-260912-mvo Q-01)
 *   ⓐ AI FAB 은 더 이상 전역이 아니다 — `chat-fab.tsx` 안에서 `usePathname()` 으로
 *      종목상세 본문(`/stocks/{code}`)만 통과시키는 경로 게이트가 섰다.
 *   ⓑ 오늘 이 바의 소비처는 `limit-chaser-form.tsx` · `vi-settings-card.tsx` **둘뿐**이고
 *      둘 다 `/trading/*` 이다. 즉 이 바와 FAB 은 **더 이상 같은 화면에 공존하지 않는다** —
 *      원래의 겹침(FAB 이 포인터 이벤트를 가로채 「수정」이 죽던 결함)은 물리적으로 불가능해졌다.
 *   ⓒ 그럼에도 `pr-[128px]` 를 남기는 이유: 이 바는 **공용 컴포넌트**다. 종목상세류 표면에
 *      새 소비처가 붙는 순간 같은 충돌이 그대로 되살아난다. z-index 를 올려 FAB 을 덮는 것은
 *      해법이 아니다 — 덮으면 이번엔 채팅 진입점이 조용히 사라진다. 오른쪽을 비우는 것이
 *      두 컨트롤이 서로를 가리지 않는 유일한 배치다.
 *   ⓓ ⚠️ **이 여백은 더 이상 테스트로 잠겨 있지 않다.** 예전에는
 *      `trading-limit-chaser.spec.ts` 케이스 4 의 좌표 단언이 FAB 폭 가정(우측 24 + 실측 83)이
 *      깨지는 순간을 잡았지만, FAB 이 그 표면에서 사라졌으므로 그 단언은 **부재 단언**으로
 *      대체됐다. 여백 숫자를 바꾸거나 지울 때 잡아 줄 기계 장치가 없다는 뜻이다 —
 *      공용 소비처가 늘어나면 이 여백의 운명을 다시 판단하라(quick-260912-mvo 후속 후보).
 */
