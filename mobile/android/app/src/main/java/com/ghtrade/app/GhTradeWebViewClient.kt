 ...
 *
 * 모든 오버라이드(`onPageStarted` 포함)는 super 를 먼저 부른다(예외 `shouldOverrideUrlLoading` — 호스트 밖 http(s) 만 가로채고 나머지는 super 결과를 그대로 반환). 로컬 에셋 요청 가로채기와 HTTP 오류 콜백은
...
class GhTradeWebViewClient(bridge: Bridge, private val host: MainActivity) : BridgeWebViewClient(bridge) {
    ...
    /**
     * D-12b (G-21-R3-4): 전체 문서 로드 시작 = 탭바 대기(첫 `route` 또는 1.5초까지). super 먼저.
     * SPA pushState 이동에는 불리지 않는다(그건 `doUpdateVisitedHistory` 만) — 탭 이동마다 탭바가 숨지 않는다.
     */
    override fun onPageStarted(view: WebView?, url: String?, favicon: Bitmap?) {
        super.onPageStarted(view, url, favicon)
        host.onDocumentStart()
    }
