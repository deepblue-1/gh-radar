 ...
 *
 * SharedPreferences `gh_trade` / 키 `theme`. 값은 `"dark"` · `"light"` 둘뿐이고 그 외·미저장은 라이트(웹 기본값).
...
object ThemeStore {
    ...
    fun load(ctx: Context): String =
        ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE).getString(KEY, "light").let { if (it == "dark") "dark" else "light" }
